...
from .adapters.base import BoundedDiagnostics, FakeTransport, PROTOCOL_VERSION, Route, Transport
...
class PendingAck:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"id": self.request_id, "epoch": self.epoch, "method": self.method,
                ...
                "expires_at_ms": self.expires_at_ms, "status": "PENDING"}
...
class ConnectionSupervisor:
    ...
    def __init__(self, shard: ShardPlan, *, transport: Transport | None = None, seed: int = 0,
                 ...
                 recycle_jitter_ms: int = 120000) -> None:
        ...
        self.epoch = 0
        self.now_ms = 0
        self.required = {s.canonical_name for s in shard.streams}
        self.acked: set[str] = set()
        self.pending: dict[int, PendingAck] = {}
        self.ack_history: deque[dict[str, Any]] = deque(maxlen=256)
        self._acknowledged_ids: deque[int] = deque(maxlen=256)
    ...
    def _pump(self) -> None:
        ...
        while self._controls:
            ...
            if method == "PONG":
                ...
                self._request_id += 1
                request = PendingAck(self._request_id, self.epoch, method, tuple(str(s).lower() for s in payload),
                                     self.now_ms, self.now_ms + self.ack_timeout_ms)
    ...
    def _fail(self, reason: str, *, uncertain: bool = False, deadline_ms: int | None = None) -> None:
        ...
        if self.counts["reconnect_attempts"] >= self.max_reconnect_attempts:
            self._next_connect_ms = None
            self._record("reconnect_budget_exhausted")
            return
        exponent = min(self.counts["reconnect_attempts"], 6)
        base = min(60000, 1000 * (2 ** exponent))
        delay = base + self.rng.randint(0, base // 5)
    ...
    def on_ack(self, message: Mapping[str, Any], *, epoch: int, now_ms: int,
               route: Route | str | None = None) -> str:
        self._time(now_ms)
        if self.state not in {ConnectionState.SUBSCRIBING, ConnectionState.ACTIVE}:
            self._record("ack_while_disconnected")
            return "UNKNOWN"
        if self._last_receive_ms is not None and self.now_ms >= self._last_receive_ms + self.idle_timeout_ms:
        ...
        if type(request_id) is not int or request_id < 1:
            ...
            return "UNKNOWN"
        if request_id in self._acknowledged_ids:
        ...
        self._acknowledged_ids.append(request_id)
        if request.method == "SUBSCRIBE":
            self.acked.update(request.streams)
        else:
            self.acked.difference_update(request.streams)
        self.ack_history.append({"id": request_id, "epoch": epoch, "status": "ACKNOWLEDGED", "stream_count": len(request.streams)})
        self._last_receive_ms = self._last_good_ms = self.now_ms
        self.counts["acknowledged_batches"] += 1
        self._pump()
        if self.required <= self.acked and not self.pending and not any(method != "PONG" for method, _ in self._controls):
    ...
    def on_frame(self, message: Any, *, epoch: int, route: Route | str, now_ms: int,
        ...
        if frame_type != "data" or type(parser_valid) is not bool or not parser_valid:
            ...
            return False
        if not isinstance(message, Mapping) or set(message) != {"stream", "data"} or not isinstance(message["stream"], str) or not isinstance(message["data"], (Mapping, list)):
            self._fail("malformed_payload", uncertain=True)
            return False
        name = message["stream"].lower()
    ...
    def update_subscriptions(self, shard: ShardPlan, *, now_ms: int) -> None:
        ...
        if shard.route != self.shard.route or shard.shard_id != self.shard.shard_id:
        ...
        self._queue_streams("UNSUBSCRIBE", (old_wire[key] for key in self.required - new))
        self._queue_streams("SUBSCRIBE", (new_wire[key] for key in new - self.required))
        self.shard, self.required = shard, new
    ...
    def step(self, now_ms: int) -> None:
        ...
        if self._recycle_at_ms is not None and now_ms >= self._recycle_at_ms:
            self._finish_coverage(self._recycle_at_ms, "scheduled_recycle")
            self._state(ConnectionState.RECYCLING, "before_24h_limit")
            self._fail("scheduled_recycle")
            return
        self._pump()
...
def run_connection_scenario(scenario: Mapping[str, Any] | str = "normal", seed: int = 0) -> dict[str, Any]:
    ...
    names = {"normal", "custom", "ack_loss", "reconnect_storm", "route_mismatch", "malformed", "late_ack", "gap", "idle_timeout", "recycle"}
    ...
    def activate(at_ms: int) -> int:
        ...
        selected_losses = set(rng.sample(range(connection._request_id + 1, connection._request_id + batches + 1), losses))
