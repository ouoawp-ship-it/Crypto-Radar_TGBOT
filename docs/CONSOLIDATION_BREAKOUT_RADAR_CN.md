...
默认候选池覆盖 Binance USDⓈ-M 全部严格认定为加密货币的活跃 USDT 永续合约，
不按 24 小时成交额淘汰长尾标的。候选入口统一拒绝稳定币、传统金融映射和无法可靠
分类的未知资产；该门禁同时约束旧多周期、自适应日线和 1H 临界预警。雷达按合约
代码稳定轮转，每批默认扫描 40 个标的；轮转游标持久化，进程重启后会从上一批继续。
升级到严格门禁时，旧口径尚未发送的 1D 汇总会停止投递并转入历史审计；正在累计的
日报只保留当前仍合格的标的，避免部署后重放稳定币内容。
