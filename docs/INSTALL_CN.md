...
项目根目录有一个 `VERSION` 文件，用来记录用户可读的版本号。当前为 `v1.59.0`，这是 v1 完整稳定版后的 Web 后台整体布局重排版本。
