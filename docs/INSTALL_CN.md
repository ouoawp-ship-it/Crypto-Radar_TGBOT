...
项目根目录有一个 `VERSION` 文件，用来记录用户可读的版本号。当前为 `v1.22.1`，后续功能更新按 `v1.22.2`、`v2.0` 这种方式递增。
