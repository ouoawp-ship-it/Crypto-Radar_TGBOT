...
默认建议只用私聊。如果开启 `AI_ALLOW_GROUP_CHAT=true`，AI 助手在群里也不会读取一句话就回复，只有别人 `@机器人用户名`、发送 `/命令@机器人用户名`，或者回复机器人消息时才会处理。
...
项目根目录有一个 `VERSION` 文件，用来记录用户可读的版本号。当前为 `v1.13.2`，后续功能更新按 `v1.13.3`、`v2.0` 这种方式递增。
