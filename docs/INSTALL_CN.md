...
Web 控制台支持运行健康度、最近错误、日志搜索筛选、推送样例预览、GitHub 更新检查、分类配置页、真实模块开关、保存前预览改动、保存后中文结果提示、最近 `.env.oi` 备份一键恢复/删除，以及结构复盘参数建议一键应用。保存成功后会自动应用新配置；主服务和结构雷达会自动重启，改了 Web 端口或 Web 令牌时 Web 控制台会短暂重启，稍后刷新页面即可。v1.30.0 起，Web 接口异常会显示统一错误卡片，提供重试、诊断报告和日志中心入口；AI 助手/价格提醒页支持局部失败展示。
...
项目根目录有一个 `VERSION` 文件，用来记录用户可读的版本号。当前为 `v1.30.0`，后续功能更新按 `v1.30.1`、`v2.0` 这种方式递增。
