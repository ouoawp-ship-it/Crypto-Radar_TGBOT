...
v1.17.0 起，私聊发送 `/start` 或 `/paopao` 会打开中文按钮首页。v1.18.0 起，首页里的「设置价格提醒」会按固定步骤执行：输入币种 -> 识别可用现货/合约 -> 手动选择交易所 -> 输入目标价 -> 确认添加。只有点击「确认添加提醒」才会真正创建提醒。
...
v1.16.0 起，AI Bot 支持自然语言查询币种雷达档案：例如“查 BTC”“GWEI 怎么看”“SOL 可以做多吗”。它会读取 `data/signal_events.json`、推送历史、启动雷达历史、结构复盘和资金费率状态，再结合当前 Binance 行情、OI、成交量、市值、流动性、结构和多交易所资金费率，输出偏多/偏空/观望/高风险观望。v1.18.0 起，AI Bot 首次打开或发送 `/start` 会显示按钮首页，价格提醒走手动选择价格源流程。`SIGNAL_EVENTS_*` 控制结构化信号索引的文件名、保留数量和保留天数，通常保持默认即可。
...
项目根目录有一个 `VERSION` 文件，用来记录用户可读的版本号。当前为 `v1.18.0`，后续功能更新按 `v1.18.1`、`v2.0` 这种方式递增。
