...
项目根目录有一个 `VERSION` 文件，用来记录用户可读的版本号。当前为 `v1.47.0`，后续收口按 `v1.48.0`、`v1.49.0`、`v1.50.0` 递增。
