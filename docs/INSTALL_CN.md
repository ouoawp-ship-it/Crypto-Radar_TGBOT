...
自然语言创建提醒必须有明确动作，比如“提醒我 / 通知我 / 设置提醒 / 到价 / 涨到 / 跌到”。只转发雷达信号会自动走数据分析；只写“BTC 跌破 58000”“ETH 突破 4200”不会自动创建价格提醒，机器人会反问确认。
...
v1.16.0 起，AI Bot 支持自然语言查询币种雷达档案：例如“查 BTC”“GWEI 怎么看”“SOL 可以做多吗”。它会读取 `data/signal_events.json`、推送历史、启动雷达历史、结构复盘和资金费率状态，再结合当前 Binance 行情、OI、成交量、市值、流动性、结构和多交易所资金费率，输出偏多/偏空/观望/高风险观望。v1.16.1 起，AI Bot 首次打开或发送 `/start` 会显示完整使用说明，说明查币、分析数据、价格提醒、提醒管理、状态问答和群内使用规则。`SIGNAL_EVENTS_*` 控制结构化信号索引的文件名、保留数量和保留天数，通常保持默认即可。
...
项目根目录有一个 `VERSION` 文件，用来记录用户可读的版本号。当前为 `v1.16.1`，后续功能更新按 `v1.16.2`、`v2.0` 这种方式递增。
