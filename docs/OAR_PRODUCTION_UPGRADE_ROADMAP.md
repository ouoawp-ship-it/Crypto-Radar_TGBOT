...
“标签库已就绪”和“当前窗口实际命中标签”必须分开报告。每次 Token Activity 都要对非铸造/销毁转账记录已分类、未分类、CEX 分类命中和明确流入/提出方向的笔数与代币量分别统计，并输出确定性覆盖率。标签库存在但本窗口全部为 `unclassified` 时，AI 继续使用受限输入；不得把全局标签数量冒充当前窗口覆盖率，也不得把交易所内部流转当成明确流入/提出方向，或把 0 流入/0 提出解释为确认没有交易所方向。
