...
盘整突破雷达及其日线、1H 临界预警和三推配置入口已移除；菜单不再提供它们的
启用或话题创建操作。旧配置值不再生效，已有状态、历史信号和群内话题保留。
...
限制。P1.1 每轮默认深度扫描 24 个：成交额前 10 个固定进入，价格或费率异动币
有界优先，至少四分之一名额永久保留给公平轮换，因此异动长期占满时普通候选也
不会饿死。七类信号互斥且不再显示未经校准的 `/100` 分数；Telegram 卡片展示
本轮选币结构、下一轮优先队列和按流通市值分层的完整候选。
