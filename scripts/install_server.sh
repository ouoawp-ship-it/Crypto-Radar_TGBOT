...
cleanup_duplicate_paoxx_nginx_servers() {
  ...
  while IFS= read -r file; do
    ...
    printf '[%s] Disabled duplicate nginx entry: %s\n' "$APP_NAME" "$file"
...
"
  ...
  printf '[%s] Nginx duplicate cleanup complete, disabled=%s, backup=%s\n' "$APP_NAME" "$disabled_count" "$backup_dir"
}
...
install_or_update_nginx_frontend_routes() {
  ...
  log "写入 Nginx 公开前台 active 反代配置: ${active_path}"
...
用法:
  ...
  bash scripts/install_server.sh config   # 修改 token / 群 ID / 话题配置
