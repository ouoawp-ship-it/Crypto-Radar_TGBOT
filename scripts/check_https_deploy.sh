...
TIMEOUT=10
CONNECT_TIMEOUT=5
PUBLIC_SLO_MS="${PUBLIC_SLO_MS:-800}"
WITH_STABLE_CHECK=0
...
check_public_intelligence_budget() {
  ...
  for offset in 0 1 2; do
    if ! curl_meta="$(curl -sS -L --compressed \
      ...
      "${BASE_URL}/public-api/radar/intelligence?window_sec=$((window_base + offset))&limit=5")"; then
      record_block "公开情报接口未在 ${TIMEOUT} 秒内完成"
      rm -f "${response_file}"
      return
    fi
    http_code="$(printf '%s' "${curl_meta}" | awk '{print $1}')"
    elapsed="$(printf '%s' "${curl_meta}" | awk '{print $3}')"
    if [ "${http_code}" != "200" ] || ! printf '%s' "${elapsed}" | grep -Eq '^[0-9]+([.][0-9]+)?$'; then
      record_block "公开情报接口性能采样失败：${curl_meta}"
      rm -f "${response_file}"
      return
    fi
    timings+=("${elapsed}")
  ...
  if awk -v elapsed="${median_elapsed}" -v limit_ms="${PUBLIC_SLO_MS}" 'BEGIN { exit !((elapsed * 1000) <= limit_ms) }'; then
    record_pass "公开情报接口 3 次冷请求中位数达标：${median_elapsed}s <= ${PUBLIC_SLO_MS}ms"
  else
    record_block "公开情报接口 3 次冷请求中位数超标：${median_elapsed}s > ${PUBLIC_SLO_MS}ms；样本=${timings[*]}"
  fi
  rm -f "${response_file}"
}
...
check_public_signal_context_actions() {
  ...
  public_ref="$(grep -ao '"public_ref"[[:space:]]*:[[:space:]]*"[^"]*"' "${signals_file}" \
    | sed -n '3p' \
    | sed -E 's/^"public_ref"[[:space:]]*:[[:space:]]*"([^"]*)"$/\1/')"
  if [ -z "${public_ref}" ]; then
    public_ref="$(grep -ao '"public_ref"[[:space:]]*:[[:space:]]*"[^"]*"' "${signals_file}" \
      | head -n 1 \
      | sed -E 's/^"public_ref"[[:space:]]*:[[:space:]]*"([^"]*)"$/\1/')"
  fi
  if ! printf '%s' "${public_ref}" | grep -Eq '^sig_[A-Za-z0-9_-]+$'; then
    record_block "最新信号缺少可用 public_ref，Telegram/Web 深链无法精确定位"
    rm -f "${signals_file}" "${context_file}"
    return
  fi
