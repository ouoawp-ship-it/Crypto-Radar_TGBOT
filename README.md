...
- 启动雷达提醒：默认 3 分钟扫描一次，推送币种、阶段、分数、市值分档、流动性分档、价格/OI/成交量变化、资金费率/结算周期和触发原因；资金费率极负会标注当前周期，例如 `-2.000%/1H`，结算周期从 8H→4H 或 4H→1H 会在信号里提示；同一币种后续更高阶段会回复上一条启动消息。
...
项目版本号写在 `VERSION` 文件里，当前为 `v1.14.1`，后续功能更新按 `v1.14.2`、`v2.0` 递增；中文菜单检查/更新时会同时显示版本号和 git 提交号。
