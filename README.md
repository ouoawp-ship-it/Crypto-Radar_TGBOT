...
- AI 币种档案：v1.16.0 新增 AI Bot 自然语言查币，v1.18.0 升级价格提醒为纯手动选择流程。发送“查 BTC”“GWEI 怎么看”“SOL 可以做多吗”时，机器人会读取历史雷达信号、当前价格/OI/成交量/资金费率/市值/流动性和结构状态，先给本地多空证据结论，AI Key 开启后再生成增强研判；设置价格提醒时会手动选择现货/合约和 Binance、Bybit、OKX、Bitget、Gate 价格源。
...
项目版本号写在 `VERSION` 文件里，当前为 `v1.19.1`，后续功能更新按 `v1.19.2`、`v2.0` 递增；中文菜单检查/更新时会同时显示版本号和 git 提交号。
