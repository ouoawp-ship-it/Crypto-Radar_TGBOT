...
这是一个只面向 Telegram 信号推送的加密市场监控项目。v2.7.0 将资金流模型升级为 P2.0：严格资产门禁、市值与流动性分档门槛、类别均衡展示，并优先复用脉冲雷达发布的 15 分钟全市场事实缓存。v2.6.1 的独立 1H 箱体临界预警、自适应 1D 盘整产品、脉冲雷达全量 15 分钟覆盖以及跨服务统一缓存/限流保持不变。
...
- 盘整突破雷达：默认关闭；旧路径继续扫描 4H、日线、周线的 24/72/240 根冻结箱体。独立的 1H 临界预警以 15m 优先、1H 兜底检查冻结上下沿，并标注同侧 4H/1D/1W 边界共振；自适应 1D 产品从 20–500 根候选长度选择最长合格箱体，用冻结日线边界接收 4H 闭合预警，并等待 1D 收盘确认。每条信号附带 K 线图，并以质量标签和具体理由代替 `/100` 评分。
