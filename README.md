...
- 启动信号生命周期：启动预警从观察、预警、确认、启动进入降温期；每个币种以一条“96根以上15m K线图 + 下方动态文字”的 Telegram 消息持续替换，保留 CoinGlass、可复制交易对和 TradingView 入口。连续降温 30 分钟后才确认失效；每个完整周期只计一个结果样本，记录最高/最低收盘变动、OI 区间、阶段耗时和结束收益；同口径样本不足 20 轮时不展示比例。
