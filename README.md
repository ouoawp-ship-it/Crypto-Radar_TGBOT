...
- v1.23.0 起，Web 控制台按单人管理员运维后台重构：菜单分为总览、AI 助手、价格提醒、雷达服务、配置中心、日志中心、检查测试、更新备份、功能说明；价格提醒从 AI 助手页拆出独立管理页，危险动作需要二次确认，总览不再直接铺大段 JSON，原始运行状态只放在高级排查折叠区。
...
项目版本号写在 `VERSION` 文件里，当前为 `v1.25.0`，后续功能更新按 `v1.25.1`、`v2.0` 递增；中文菜单检查/更新时会同时显示版本号和 git 提交号。
