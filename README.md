...
这是一个只面向 Telegram 信号推送的加密市场监控项目。项目保留脉冲雷达、资金流雷达、资金摘要、资金费率警报和公告风险等独立模块。
...
盘整突破雷达已移除，包括旧多周期箱体、自适应日线、日线日报、1H 临界预警及其三推功能。
旧配置项不再启用计算或发送；原有历史信号、状态文件和 Telegram 话题保留，
更新不会自动删除历史数据或群内消息。脉冲雷达的突破分类和图表能力继续保留。
...
`--send --confirm-real-send`；它只刷新脉冲雷达。`pulse-review-report` 只读取已回填的本地复盘数据，
...
bash scripts/deploy_tag.sh --check-tag v2.7.2
bash scripts/deploy_tag.sh --tag v2.7.2 --yes
```
...
现有五个核心雷达可分别经二次确认暂停或恢复自动调度。公共市场快照和主进程不会
