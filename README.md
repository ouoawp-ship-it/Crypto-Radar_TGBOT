...
这是一个只面向 Telegram 信号推送的加密市场监控项目。v2.7.1 将盘整突破雷达的旧多周期、自适应 1D 和 1H 临界预警候选池统一接入严格资产门禁，排除稳定币、传统金融映射和未知资产。v2.7.0 的资金流 P2.0、市值与流动性分档、类别均衡展示以及全市场事实缓存保持不变。
...
- 盘整突破雷达：严格排除稳定币、传统金融映射和未知资产；旧路径继续扫描 4H、日线、周线的 24/72/240 根冻结箱体。独立的 1H 临界预警以 15m 优先、1H 兜底检查冻结上下沿，并标注同侧 4H/1D/1W 边界共振；自适应 1D 产品从 20–500 根候选长度选择最长合格箱体，用冻结日线边界接收 4H 闭合预警，并等待 1D 收盘确认。每条信号附带 K 线图，并以质量标签和具体理由代替 `/100` 评分。
...
资金流 P2.0 优先复用脉冲雷达每 15 分钟发布到统一市场快照的全市场分析结果，
在整点把四段连续快照聚合成严格闭合的 1 小时窗口；共享快照完整时覆盖全部合格
候选且不再发起逐币深度 REST 请求。只有快照缺失的币才进入最多 24 个的 REST
回退队列：成交额前 10 个固定进入，24 小时价格绝对变化不少于 2% 或资金费率
绝对值不少于 0.05% 的币优先，其余始终至少保留四分之一名额公平轮换。异动币
再多也不能长期挤掉普通缺口候选。默认最多展示 8 条，七类按固定优先级互斥判断，
`FLOW_MIN_SCORE` 仅为旧环境兼容保留，不再参与过滤。候选入口严格拒绝稳定币、
TradFi 映射、身份不明资产和没有 Binance 现货交易对的合约；合格加密合约再按
超大/大/中/小市值与高/中/低流动性
组合调整有效门槛，市值缺失单列为“待补全”且采用保守门槛。高流动性币要求更大
绝对资金净额，低流动性币要求更高主动净占比，避免用同一阈值偏袒某一类币。
REST 回退时，同一窗口的价格与合约 CVD 复用一份 K 线；展示按类别轮转取样，
风险类别不会被大量同类信号全部挤出。共享快照写入属于行情事实缓存；dry-run
只预览候选轮换，不推进生产信号状态。

盘整突破雷达使用独立模板 `TG_CONSOLIDATION_BREAKOUT` 和严格话题路由，升级后
默认关闭，不会改变现有生产流量；三推、自适应日线和 1H 箱体临界预警分别使用
独立子开关，后两者还默认保持影子模式。1H 临界预警用全市场慢速发现冻结箱体，
再用近边界活跃集合检查已闭合 15m/1H K线；同方向 4H/1D/1W 冻结边界重合时会
明确标注共振，但不会把临近写成已经突破。首次启用、创建专属话题、20–500 日
自适应箱体、全市场轮转、假突破/三推状态机和回滚方式见
...
bash scripts/deploy_tag.sh --check-tag v2.7.1
bash scripts/deploy_tag.sh --tag v2.7.1 --yes
```
...
`CONSOLIDATION_BREAKOUT_THREE_PUSH_ENABLE` 单独控制。1H 临界预警按
`HOURLY_PROXIMITY_ENABLE → SHADOW 验收 → 关闭 SHADOW` 上线；自适应日线按
