...
function isWithin(value: string | undefined, hours: number) {
  const date = dateOf(value);
  return Boolean(date && Date.now() - date.getTime() <= hours * 3_600_000);
}
...
function FeedItem({ item, mode }: { item: NewsEvent; mode: FeedMode }) {
  ...
  const body = <article className={`border-b border-border-subtle px-2 py-2 transition-colors hover:bg-primary-50/45 ${risk ? "border-l-2 border-l-risk" : opportunity ? "border-l-2 border-l-good" : "border-l-2 border-l-transparent"}`}>
    {mode === "kol" ? <div className="mb-1 flex items-center gap-1 text-[7px] text-text-muted"><span className="font-mono">{relativeClock(item.published_at)}</span><b className="min-w-0 flex-1 truncate text-warn">{item.source || "@KOL"}</b>{score >= 100 ? <span className="rounded-[2px] bg-warn/10 px-1 py-px font-semibold text-warn">🔥 {score}</span> : null}</div> : <div className="mb-1 flex items-center gap-1 text-[7px] text-text-muted"><span className={`min-w-0 flex-1 truncate ${mode === "english" ? "font-semibold text-primary-600" : ""}`}>{item.source || "公开来源"}</span>{item.importance === "high" ? <span className="rounded-[2px] bg-risk/10 px-1 py-px font-semibold text-risk">高影响</span> : null}</div>}
    <h3 className={`${mode === "english" ? "line-clamp-4" : "line-clamp-3"} text-[10px] font-medium leading-[1.52] text-text-primary`}>{item.title || item.summary || "未命名资讯"}</h3>
    {mode !== "kol" && item.summary && item.summary !== item.title ? <p className="mt-1 line-clamp-2 text-[8px] leading-[1.5] text-text-secondary">{item.summary}</p> : null}
    <div className="flex items-end justify-between gap-1"><SymbolTags symbols={item.symbols}/>{mode === "kol" && engagement ? <span className="ml-auto shrink-0 text-[7px] text-text-muted">赞 {engagement.likes || 0} · 转 {engagement.reposts || 0}</span> : null}</div>
  </article>;
...
function GroupedNews({ items }: { items: NewsEvent[] }) {
  ...
  return <>{[...groups.entries()].map(([hour, rows]) => <div key={hour}><div className="flex h-6 items-center justify-between border-b border-border-subtle bg-surface-low px-2 text-[7px] text-text-muted"><span className="font-mono font-semibold">{hour}</span><span>{rows.length} 条</span></div>{rows.map((item, index) => <div className="grid grid-cols-[34px_minmax(0,1fr)]" key={item.event_id || `${item.title}-${index}`}><span className="border-b border-border-subtle py-2 text-center font-mono text-[7px] text-text-muted">{clock(item.published_at)}</span><FeedItem item={item} mode="news"/></div>)}</div>)}</>;
}
...
function InfoColumn({ title, payload, loading, showDigest, mode }: { title: string; payload?: InfoFeedPayload; loading: boolean; showDigest: boolean; mode: FeedMode }) {
  ...
  return <section className="workstation-panel flex min-h-0 min-w-0 flex-col">
    <div className="grid h-[40px] shrink-0 grid-cols-[auto_minmax(72px,1fr)_auto] items-center gap-1.5 border-b border-border-subtle bg-surface-low px-2 min-[1024px]:h-[30px]"><h2 className="flex whitespace-nowrap text-[10px] font-bold text-text-primary"><span className="mr-1 text-primary-600">{icon}</span>{title}</h2><div className="relative"><span className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-[8px] text-text-muted">⌕</span><input aria-label={`搜索${title}`} className="h-6 w-full rounded-[3px] border border-border-subtle bg-surface-panel pl-5 pr-1.5 text-[8px] text-text-primary outline-none placeholder:text-text-muted focus:border-primary-500" onChange={(event) => setQuery(event.target.value)} placeholder="搜索…" value={query}/></div><LiveBadge payload={payload}/></div>
    {showDigest ? <button aria-expanded={expanded} className={`${expanded ? "min-h-[72px]" : "h-[43px] min-[1024px]:h-[30px]"} grid shrink-0 grid-cols-[38px_minmax(0,1fr)_auto] items-start gap-1 border-b border-border-subtle bg-[#faf8ff] px-2 py-1.5 text-left`} onClick={() => setExpanded((value) => !value)} type="button"><span className="rounded-[2px] bg-primary-50 px-1 py-0.5 text-center text-[7px] font-semibold text-primary-600">AI 解读</span><span className={`${expanded ? "" : "line-clamp-2"} text-[8px] leading-[1.45] text-text-secondary`}>{channelDigest(payload)}</span><span className="text-[7px] font-semibold text-warn">{expanded ? "收起" : "展开⌄"}</span></button> : null}
...
function PlazaColumn({ payload, loading, showDigest }: { payload?: InfoFeedPayload; loading: boolean; showDigest: boolean }) {
  ...
  return <section className="workstation-panel flex min-h-0 min-w-0 flex-col"><div className="flex h-[40px] shrink-0 items-center justify-between border-b border-border-subtle bg-surface-low px-2 min-[1024px]:h-[30px]"><h2 className="flex items-center text-[10px] font-bold text-text-primary"><span className="mr-1 text-warn">▣</span>市场广场情绪</h2><LiveBadge payload={payload}/></div>{showDigest ? <div className="flex h-[43px] shrink-0 items-start gap-1 border-b border-border-subtle bg-[#fffaf0] px-2 py-1.5 min-[1024px]:h-[30px]"><span className="rounded-[2px] bg-warn/10 px-1 py-0.5 text-[7px] font-semibold text-warn">AI 解读</span><p className="line-clamp-2 flex-1 text-[8px] leading-[1.45] text-text-secondary">公开社交流按币种聚合，结合帖子方向与互动强度生成 4h 活力榜和 24h 总榜。</p></div> : null}<div className="workstation-scroll min-h-0 flex-1 overflow-auto"><div className="flex h-7 items-center justify-between border-b border-border-subtle bg-surface-low px-2"><h3 className="text-[9px] font-bold">▧ 4h 活力榜</h3><span className="text-[7px] text-text-muted">快速更新 · Top 8</span></div>{active.slice(0, 8).map((item, index) => <div className="grid h-[34px] grid-cols-[18px_18px_minmax(0,1fr)_auto] items-center gap-1 border-b border-border-subtle px-2 text-[8px]" key={item.symbol}><span className="font-mono text-warn">{String(index + 1).padStart(2, "0")}</span><CoinIcon coin={item.symbol}/><span className="font-semibold">${item.symbol}<small className="ml-1 font-normal text-text-muted">多 {item.positive} · 空 {item.negative}</small></span><span className="font-mono font-semibold text-good">{item.count} 帖</span></div>)}{!loading && !active.length ? <div className="grid h-20 place-items-center text-[8px] text-text-muted">公开社交源暂无 4h 币种样本</div> : null}<div className="flex h-7 items-center justify-between border-y border-border-subtle bg-surface-low px-2"><h3 className="text-[9px] font-bold">◉ 24h 总榜</h3><span className="text-[7px] text-text-muted">情绪分析</span></div>{total.slice(0, 8).map((item, index) => { const sentiment = item.positive >= item.negative ? "偏多" : "偏空"; const ratio = Math.round(100 * Math.max(item.positive, item.negative) / Math.max(1, item.positive + item.negative)); return <article className="border-b border-border-subtle px-2 py-2" key={item.symbol}><div className="flex items-center gap-1.5"><span className="font-mono text-[8px] text-warn">{String(index + 1).padStart(2, "0")}</span><CoinIcon coin={item.symbol}/><b className="text-[9px]">${item.symbol}</b><span className={`rounded-[2px] px-1 text-[7px] ${sentiment === "偏多" ? "bg-good/10 text-good" : "bg-risk/10 text-risk"}`}>{sentiment} {ratio}%</span><span className="ml-auto text-[7px] text-text-muted">{item.count} 帖</span></div><p className="mt-1 line-clamp-2 text-[8px] leading-[1.45] text-text-secondary">{item.summary}</p></article>; })}</div></section>;
}
