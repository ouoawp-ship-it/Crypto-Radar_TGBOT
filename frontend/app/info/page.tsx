...
function ChannelIcon({ mode }: { mode: FeedMode | "plaza" }) {
  ...
  const wideIcon = mode === "english" ? <svg className="relative hidden h-4 w-4 min-[1280px]:block" viewBox="0 0 12 12"><path d="M3 1.8 10 6 3 10.2Z" fill="currentColor"/></svg>
    : mode === "kol" ? <svg className="relative hidden h-4 w-4 min-[1280px]:block" fill="none" viewBox="0 0 14 14"><circle cx="5" cy="4.2" r="2" fill="currentColor"/><circle cx="9.5" cy="5" r="1.6" fill="currentColor" opacity=".72"/><path d="M1.8 11c.2-2.3 1.5-3.5 3.3-3.5S8.2 8.7 8.4 11H1.8Zm6.3 0c-.1-1.1-.5-2-1.1-2.7.6-.5 1.3-.7 2.1-.7 1.7 0 2.8 1.1 3 3.4h-4Z" fill="currentColor"/></svg>
      : mode === "plaza" ? <svg className="relative hidden h-4 w-4 min-[1280px]:block" fill="none" viewBox="0 0 12 12"><path d="M2 2h8v8H2z" stroke="currentColor"/><path d="M4 4h4v4H4z" fill="currentColor" opacity=".48"/></svg>
        : <svg className="relative hidden h-4 w-4 min-[1280px]:block" fill="none" viewBox="0 0 12 12"><path d="M3 1.5h6v9H3z" stroke="currentColor"/><path d="M4.5 4h3M4.5 6h3M4.5 8h2" stroke="currentColor" strokeLinecap="round"/></svg>;
  return <span aria-hidden="true" className={`relative grid h-[22px] w-[22px] shrink-0 place-items-center rounded-[3px] text-[14px] font-extrabold leading-none before:hidden min-[1280px]:h-6 min-[1280px]:w-6 min-[1280px]:bg-transparent min-[1280px]:before:absolute min-[1280px]:before:left-0 min-[1280px]:before:top-px min-[1280px]:before:block min-[1280px]:before:h-[22px] min-[1280px]:before:w-[22px] min-[1280px]:before:rounded-[3px] ${colors}`} data-testid={testId}><span className="relative min-[1280px]:hidden">{icon}</span>{wideIcon}</span>;
}
...
function InfoColumn({ title, payload, brief, loading, showDigest, mode }: { title: string; payload?: InfoFeedPayload; brief?: InfoBrief; loading: boolean; showDigest: boolean; mode: FeedMode }) {
  ...
  return <section className="workstation-panel flex min-h-0 min-w-0 flex-col rounded-[5px]">
    <div className={`${headerHeight} flex shrink-0 items-center gap-2 border-b border-border-subtle bg-surface-low px-3 min-[1280px]:gap-3 min-[1280px]:pl-3.5 min-[1280px]:pr-4`}><h2 className={`flex items-center gap-2 text-[15px] font-bold leading-tight text-text-primary ${mode === "english" ? "max-w-[96px] whitespace-normal min-[1280px]:max-w-none min-[1280px]:whitespace-nowrap" : "whitespace-nowrap"}`}><ChannelIcon mode={mode}/>{title}</h2>{mode !== "kol" ? <div className="relative ml-auto w-[145px] shrink-0 min-[1280px]:w-[147px]"><span className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-[13px] text-text-muted">🔍</span><input aria-label={`搜索${title}`} className="h-[27px] w-full rounded-[5px] border border-[#cbd2dc] bg-surface-container pl-8 pr-2 text-[12px] text-text-primary outline-none placeholder:text-text-muted focus:border-primary-500" onChange={(event) => setQuery(event.target.value)} placeholder="搜索…" value={query}/></div> : <span className="ml-auto"/>}<LiveBadge payload={payload}/></div>
...
function PlazaColumn({ payload, brief, loading, showDigest }: { payload?: InfoFeedPayload; brief?: InfoBrief; loading: boolean; showDigest: boolean }) {
  ...
  return <section className="workstation-panel flex min-h-0 min-w-0 flex-col rounded-[5px]">
    ...
    {showDigest ? <button aria-expanded={expanded} className={`${expanded ? "min-h-[96px]" : "h-[38px] min-[1280px]:h-[39px]"} flex shrink-0 items-start gap-2 border-b border-border-subtle bg-[#fbf8f1] px-3 py-2 text-left min-[1280px]:px-3.5`} data-testid="info-plaza-digest" onClick={() => setExpanded((value) => !value)} title={brief?.model_generated ? "模型生成摘要" : "规则蒸馏摘要，未冒充模型结论"} type="button">
