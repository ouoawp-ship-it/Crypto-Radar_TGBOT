...
function ChannelIcon({ mode }: { mode: FeedMode | "plaza" }) {
  ...
  const icon = mode === "english" ? "▶" : mode === "kol" ? "👥" : mode === "plaza" ? "▣" : "≡";
  const colors = mode === "english" ? "bg-[#ece9f7] text-[#7a5fd0] min-[1600px]:before:bg-[#ece9f7]" : mode === "kol" ? "bg-[#f3e9d9] text-[#383838] min-[1600px]:before:bg-[#f3e9d9]" : mode === "plaza" ? "bg-[#f3ecdd] text-[#c39a33] min-[1600px]:before:bg-[#f3ecdd]" : "bg-[#e2eef1] text-[#2c8d9b] min-[1600px]:before:bg-[#e2eef1]";
  const wideIcon = mode === "english" ? <svg className="relative hidden h-4 w-4 min-[1600px]:block" viewBox="0 0 12 12"><path d="M3 1.8 10 6 3 10.2Z" fill="currentColor"/></svg>
    : mode === "kol" ? <svg className="relative hidden h-4 w-4 min-[1600px]:block" fill="none" viewBox="0 0 14 14"><circle cx="5" cy="4.2" r="2" fill="currentColor"/><circle cx="9.5" cy="5" r="1.6" fill="currentColor" opacity=".72"/><path d="M1.8 11c.2-2.3 1.5-3.5 3.3-3.5S8.2 8.7 8.4 11H1.8Zm6.3 0c-.1-1.1-.5-2-1.1-2.7.6-.5 1.3-.7 2.1-.7 1.7 0 2.8 1.1 3 3.4h-4Z" fill="currentColor"/></svg>
      : mode === "plaza" ? <svg className="relative hidden h-4 w-4 min-[1600px]:block" fill="none" viewBox="0 0 12 12"><path d="M2 2h8v8H2z" stroke="currentColor"/><path d="M4 4h4v4H4z" fill="currentColor" opacity=".48"/></svg>
        : <svg className="relative hidden h-4 w-4 min-[1600px]:block" fill="none" viewBox="0 0 12 12"><path d="M3 1.5h6v9H3z" stroke="currentColor"/><path d="M4.5 4h3M4.5 6h3M4.5 8h2" stroke="currentColor" strokeLinecap="round"/></svg>;
  return <span aria-hidden="true" className={`relative grid h-[22px] w-[22px] shrink-0 place-items-center rounded-[3px] text-[14px] font-extrabold leading-none before:hidden min-[1600px]:h-6 min-[1600px]:w-6 min-[1600px]:bg-transparent min-[1600px]:before:absolute min-[1600px]:before:left-0 min-[1600px]:before:top-px min-[1600px]:before:block min-[1600px]:before:h-[22px] min-[1600px]:before:w-[22px] min-[1600px]:before:rounded-[3px] ${colors}`} data-testid={testId}><span className="relative min-[1600px]:hidden">{icon}</span>{wideIcon}</span>;
}
...
function LiveBadge({ payload }: { payload?: InfoFeedPayload }) {
  ...
  return <span className={`inline-flex h-[21px] items-center gap-1.5 rounded-full px-3 text-[11px] font-bold min-[1600px]:h-[22px] ${ready ? "bg-[#def1ea] text-good" : "bg-warn/10 text-warn"}`}><span className={`h-1.5 w-1.5 rounded-full ${ready ? "animate-pulse bg-good" : "bg-warn"}`}/>{ready ? "LIVE" : "SYNC"}</span>;
}
...
function SymbolTags({ symbols }: { symbols?: string[] }) {
  ...
  return <div className="mt-2 flex flex-wrap items-center gap-1.5">{symbols.slice(0, 4).map((symbol) => {
    ...
    return <span className="inline-flex items-center gap-1 rounded-full border border-[#dfe2e6] bg-[#eef0f3] px-2 py-0.5 font-mono text-[11px] font-semibold text-text-secondary min-[1600px]:text-[10px]" key={symbol}><span aria-hidden="true" className="grid h-3 w-3 place-items-center text-[8px] font-extrabold text-text-primary">{coin.slice(0, 1)}</span>${coin}</span>;
  })}</div>;
}
...
function FeedItem({ item, mode }: { item: NewsEvent; mode: FeedMode }) {
  ...
  const streamSpacing = mode === "english"
    ? "pb-[11px] pl-[8px] pr-[7px] pt-[5px] min-[1600px]:pb-3 min-[1600px]:pl-[19px] min-[1600px]:pr-[13px] min-[1600px]:pt-2"
    : "pb-2.5 pl-[6px] pr-[10px] pt-1.5 min-[1600px]:pb-[13px] min-[1600px]:pl-[19px] min-[1600px]:pr-[13px] min-[1600px]:pt-[7px]";
  const content = mode === "news" ? <article className="border-b border-border-subtle pb-[7px] pl-2.5 pr-1.5 pt-[9px] transition-colors hover:bg-primary-50/45 min-[1600px]:px-3 min-[1600px]:py-[11px]" data-info-row="news">
  ...
  const body = mode === "news" ? content : <div className={`grid grid-cols-[62px_minmax(0,1fr)] border-b border-border-subtle ${mode === "kol" ? "min-h-[60px] min-[1600px]:min-h-[66px]" : "min-h-[84px] min-[1600px]:min-h-[87px]"}`} data-info-row={mode}><span className="border-border-subtle py-3 text-center font-mono text-[11px] text-text-muted">{mode === "kol" ? relativeClock(item.published_at) : clock(item.published_at)}</span>{content}</div>;
...
function GroupedNews({ items }: { items: NewsEvent[] }) {
  ...
  return <>{[...groups.entries()].map(([hour, rows]) => <div key={hour}><div className="flex h-[29px] items-center justify-between border-b border-border-subtle bg-surface-container px-3 text-[10px] text-text-muted"><span className="font-mono font-semibold">{hour}</span><span>{rows.length} 条</span></div>{rows.map((item, index) => <div className="grid grid-cols-[54px_minmax(0,1fr)] min-[1600px]:grid-cols-[62px_minmax(0,1fr)]" key={item.event_id || `${item.title}-${index}`}><span className="border-b border-border-subtle pb-[7px] pt-[9px] text-center font-mono text-[11px] text-text-muted min-[1600px]:pb-[11px] min-[1600px]:pt-[11px]">{clock(item.published_at)}</span><FeedItem item={item} mode="news"/></div>)}</div>)}</>;
}
...
function InfoColumn({ title, payload, brief, loading, showDigest, mode }: { title: string; payload?: InfoFeedPayload; brief?: InfoBrief; loading: boolean; showDigest: boolean; mode: FeedMode }) {
  ...
  const headerHeight = mode === "english" ? "h-[66px] min-[1600px]:h-[52px]" : mode === "kol" ? "h-[44px] min-[1600px]:h-[47px]" : "h-[47px] min-[1600px]:h-[52px]";
  const digestHeight = mode === "english" ? "h-[58px] min-[1600px]:h-[39px]" : "h-[59px] min-[1600px]:h-[39px]";
  return <section className="workstation-panel flex min-h-0 min-w-0 flex-col rounded-[5px]">
    <div className={`${headerHeight} flex shrink-0 items-center gap-2 border-b border-border-subtle bg-surface-low px-3 min-[1600px]:gap-3 min-[1600px]:pl-3.5 min-[1600px]:pr-4`}><h2 className={`flex items-center gap-2 text-[15px] font-bold leading-tight text-text-primary ${mode === "english" ? "max-w-[96px] whitespace-normal min-[1600px]:max-w-none min-[1600px]:whitespace-nowrap" : "whitespace-nowrap"}`}><ChannelIcon mode={mode}/>{title}</h2>{mode !== "kol" ? <div className="relative ml-auto w-[145px] shrink-0 min-[1600px]:w-[147px]"><span className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-[13px] text-text-muted">🔍</span><input aria-label={`搜索${title}`} className="h-[27px] w-full rounded-[5px] border border-[#cbd2dc] bg-surface-container pl-8 pr-2 text-[12px] text-text-primary outline-none placeholder:text-text-muted focus:border-primary-500" onChange={(event) => setQuery(event.target.value)} placeholder="搜索…" value={query}/></div> : <span className="ml-auto"/>}<LiveBadge payload={payload}/></div>
    {showDigest ? <button aria-expanded={expanded} className={`${expanded ? "min-h-[96px]" : digestHeight} ${digestSurface} grid shrink-0 grid-cols-[48px_minmax(0,1fr)_auto] items-start gap-2 border-b border-border-subtle px-3 py-3 text-left min-[1600px]:grid-cols-[44px_minmax(0,1fr)_auto] min-[1600px]:gap-3 min-[1600px]:px-3.5 min-[1600px]:py-2`} onClick={() => setExpanded((value) => !value)} title={brief?.model_generated ? "模型生成摘要" : "规则蒸馏摘要，未冒充模型结论"} type="button"><span className={`${digestBadge} ${digestBadgeSpacing} rounded-[3px] text-center text-[10px] font-semibold`}>AI 解读</span><span className={`${expanded ? "" : "line-clamp-2"} text-[12px] leading-[1.55] text-text-secondary`}>{channelDigest(payload, brief)}</span><span className="text-[10px] font-semibold text-warn">{expanded ? "收起" : "展开⌄"}</span></button> : null}
...
function PlazaColumn({ payload, brief, loading, showDigest }: { payload?: InfoFeedPayload; brief?: InfoBrief; loading: boolean; showDigest: boolean }) {
  ...
  return <section className="workstation-panel flex min-h-0 min-w-0 flex-col rounded-[5px]">
    <div className="flex h-[44px] shrink-0 items-center justify-between border-b border-border-subtle bg-surface-low px-3 min-[1600px]:h-[47px] min-[1600px]:pl-3.5 min-[1600px]:pr-4">
    ...
    {showDigest ? <button aria-expanded={expanded} className={`${expanded ? "min-h-[96px]" : "h-[38px] min-[1600px]:h-[39px]"} flex shrink-0 items-start gap-2 border-b border-border-subtle bg-[#fbf8f1] px-3 py-2 text-left min-[1600px]:px-3.5`} data-testid="info-plaza-digest" onClick={() => setExpanded((value) => !value)} title={brief?.model_generated ? "模型生成摘要" : "规则蒸馏摘要，未冒充模型结论"} type="button">
      <span className="whitespace-nowrap rounded-[3px] border border-[#f0dfc0] px-[5px] py-0.5 text-[10px] font-semibold text-warn" title={brief?.model_generated ? "模型生成摘要" : "规则蒸馏摘要，未冒充模型结论"}>AI 解读</span>
    ...
    <div className="workstation-scroll min-h-0 flex-1 overflow-auto">
      <div className="flex h-[40px] items-center justify-between border-b border-border-subtle bg-surface-container px-[14px] min-[1600px]:px-[18px]">
      ...
      {active.slice(0, 8).map((item, index) => {
        ...
        return <div className="grid h-[50.4px] grid-cols-[24px_27px_minmax(0,1fr)_auto] items-center gap-2 border-b border-border-subtle px-4 py-1.5 text-[11px] min-[1600px]:h-[54px] min-[1600px]:px-5" key={item.symbol || coin}>
      ...
      <div className="-mt-0.5 flex h-[39px] items-center justify-between border-y border-border-subtle bg-surface-container px-[14px] min-[1600px]:mt-0 min-[1600px]:px-[18px]">
      ...
      {total.slice(0, 8).map((item, index) => {
        ...
        return <article className={`${index === 0 ? "bg-[#faf6ee]" : ""} min-h-[129px] border-b border-border-subtle px-3 py-2.5 min-[1600px]:min-h-[130px]`} key={item.symbol || coin}>
...
export default function InfoPage() {
  ...
  return <div aria-busy={loading} className="workstation-page mercu-info-grid" data-testid="info-workstation">
    <section className="workstation-panel -mx-2 flex h-[67px] shrink-0 items-center gap-3.5 rounded-none border-x-0 border-t-0 px-[17px] max-[767px]:mx-0 min-[1600px]:-mx-[14px] min-[1600px]:h-[71px] min-[1600px]:px-[23px]"><div className="flex h-[46px] w-[46px] shrink-0 items-center justify-center rounded-[6px] bg-surface-container" data-testid="info-digest-icon"><svg aria-hidden="true" className="h-6 w-6" fill="none" viewBox="0 0 24 24"><path d="M13 2.5c.45 4.08 2.42 6.07 6.5 6.5-4.08.45-6.05 2.42-6.5 6.5-.45-4.08-2.42-6.05-6.5-6.5 4.08-.43 6.05-2.42 6.5-6.5Z" fill="#c28a0f"/><circle cx="5.1" cy="15.7" r="1.45" fill="#6079b8"/><circle cx="18.7" cy="4.3" r="1.15" fill="#d6a72e"/></svg></div><div><div className="flex items-center gap-[5px]"><h1 aria-label="AI信息蒸馏" className="text-[18px] font-bold text-text-primary">AI 信息蒸馏</h1><span className="rounded-[3px] border border-[#e5d2a5] bg-white px-2 py-0.5 text-[10px] font-semibold text-[#b8860b]">引擎 v2.4</span></div><p className="text-[12px] text-text-muted">全量聚合全网信息 · 实时增量更新<button className="ml-5 underline decoration-dotted" onClick={() => setDisclaimer(true)} type="button">免责声明</button></p></div><div className="ml-auto flex items-center gap-2"><span className="sr-only">高影响 {high} · 更新 {clock(updatedAt)}</span><button aria-pressed={showDigest} className="h-[38px] w-[197px] rounded-full border border-[#e1cb96] bg-[#f6efdf] text-[13px] font-semibold text-[#b8860b] min-[1600px]:h-[40px] min-[1600px]:w-[201px]" disabled={loading} onClick={() => { setShowDigest((value) => !value); void load(true); }} type="button">{loading ? "分析中…" : "✦ 4h AI 综合分析 •⌄"}</button></div></section>
    {error ? <div className="border border-risk/20 bg-risk/5 px-3 py-1 text-[8px] text-risk">{error}</div> : null}
    <main className="grid min-h-0 grid-cols-4 gap-2 min-[1600px]:gap-[14px]" data-testid="info-four-columns"><InfoColumn brief={briefByChannel.news} loading={loading} mode="news" payload={feeds.news} showDigest={showDigest} title="聚合资讯"/><InfoColumn brief={briefByChannel.en} loading={loading} mode="english" payload={feeds.english} showDigest={showDigest} title="英文流资讯"/><InfoColumn brief={briefByChannel.kol} loading={loading} mode="kol" payload={feeds.kol} showDigest={showDigest} title="KOL聚合资讯"/><PlazaColumn brief={briefByChannel.plaza} loading={loading} payload={feeds.plaza} showDigest={showDigest}/></main>
