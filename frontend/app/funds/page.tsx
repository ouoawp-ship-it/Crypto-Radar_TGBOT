...
function SectorBubbleChart({ payload }: { payload: FundsSectorsPayload }) {
  ...
  const bubble = (item: (typeof sectors)[number], index: number, top: boolean) => {
    ...
    const valueVisibility = `${showCompactValue ? "block" : "hidden"} ${showWideValue ? "min-[1600px]:block" : "min-[1600px]:hidden"}`;
  ...
  return <div className="funds-sector-stage relative min-h-[320px] flex-1 overflow-hidden"><div className="absolute left-2 top-2 text-[7px] font-semibold text-good">流入 ↑</div><div className="absolute bottom-2 left-2 text-[7px] font-semibold text-risk">流出 ↓</div><span aria-hidden="true" className="absolute inset-x-2 top-1/2 border-t border-dashed border-border-subtle"/>{positive.map((item, index) => bubble(item, index, true))}{negative.map((item, index) => bubble(item, index, false))}{!sectors.length ? <div className="grid h-full place-items-center text-[9px] text-text-muted">板块资金真实样本正在积累</div> : null}<span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-surface-panel px-1 text-[7px] tracking-[.18em] text-text-muted/60">PaoXX 数据</span></div>;
}
...
function SectorOverview({ payload, windowSec, onWindow }: { payload: FundsSectorsPayload; windowSec: number; onWindow: (value: number) => void }) {
  ...
  return <section className="workstation-panel flex min-h-0 flex-col [&>.workstation-panel-header]:h-[59px] [&>.workstation-panel-header]:bg-[#fbfcfc] [&>.workstation-panel-header]:px-4 min-[1600px]:[&>.workstation-panel-header]:h-[60px]">
    <PanelTitle action={<div className="flex h-[34px] w-[188px] translate-y-px gap-0 rounded-[4px] border border-border-subtle bg-[#f1f4f8] p-1 min-[1600px]:w-[190px] min-[1600px]:-translate-x-0.5 min-[1600px]:translate-y-0.5 min-[1600px]:gap-1">{SECTOR_WINDOWS.map((item) => <button aria-pressed={windowSec === item.value} className={`h-full flex-1 rounded-[3px] px-0 text-[12px] font-semibold ${windowSec === item.value ? "bg-[#d9e7ed] text-[#1696a7] ring-1 ring-[#8ac8d2]" : "text-text-muted"}`} key={item.value} onClick={() => onWindow(item.value)} type="button">{item.label}</button>)}</div>} title="板块资金流"/>
    <div className="h-[123px] shrink-0 border-b border-border-subtle bg-[#fcfcfd] px-4 py-1.5 min-[1600px]:h-[127px] min-[1600px]:px-[17px]">
      ...
      <div className="mt-2 flex -translate-y-px gap-2 whitespace-nowrap text-[10px] text-text-muted"><span className="flex h-[21px] w-[128px] shrink-0 items-center rounded-[3px] bg-[#f1f4f8] px-1.5 min-[1600px]:w-[143px]">领涨 <span className="ml-1 font-mono text-good">{summary.leading_inflow_sector || "—"} {leadingInflow ? cnMoney(leadingInflow.net_flow_usd) : ""}</span></span><span className="flex h-[21px] w-[152px] shrink-0 items-center rounded-[3px] bg-[#f1f4f8] px-1.5 min-[1600px]:w-[157px]">领跌 <span className="ml-1 font-mono text-risk">{summary.leading_outflow_sector || "—"} {leadingOutflow ? cnMoney(leadingOutflow.net_flow_usd) : ""}</span></span></div>
    </div>
...
function AssetsOverview({ assets, payload, query, setQuery, windowSec, onWindow, onSelect, onPage, loading, error, sortKey, direction, onSort, favorites, onFavorite }: {
  ...
  const columns = "grid-cols-[54px_240px_188px_64px_105px_61px_65px_68px_78px_48px_63px] min-[1600px]:grid-cols-[58px_245px_192px_repeat(8,minmax(0,1fr))]";
  ...
  return <section className="workstation-panel flex min-h-0 min-w-0 flex-col" data-testid="funds-assets-overview">
    <div className="flex h-[65px] shrink-0 items-start gap-3 border-b border-border-subtle bg-surface-panel px-0 min-[1600px]:h-[68px]">
      <div className="mt-[3px] flex h-[42px] w-[390px] shrink-0 overflow-hidden rounded-[5px] border border-border-subtle bg-[#f1f4f8] p-[5px] min-[1600px]:h-[44px] min-[1600px]:w-[465px] min-[1600px]:p-[6px]">{ASSET_WINDOWS.map((item) => <button aria-pressed={windowSec === item.value} className={`h-full flex-1 rounded-[4px] text-[13px] font-semibold first:min-w-[80px] min-[1600px]:first:min-w-[94px] ${windowSec === item.value ? "bg-[#d9e7ed] text-[#1696a7] ring-1 ring-[#8ac8d2]" : "text-text-muted"}`} key={item.value} onClick={() => onWindow(item.value)} type="button">{item.label}</button>)}</div>
      <div className="relative -mt-px ml-auto w-[398px] shrink-0 min-[1600px]:w-[420px]"><svg aria-hidden="true" className="absolute left-4 top-1/2 h-[17px] w-[17px] -translate-y-1/2 text-text-muted" fill="none" viewBox="0 0 16 16"><circle cx="7" cy="7" r="4.5" stroke="currentColor" strokeWidth="1.5"/><path d="m10.5 10.5 3 3" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5"/></svg><input aria-label="搜索全体代币" className="h-12 w-full rounded-[5px] border border-border-subtle bg-[#f1f4f8] pl-11 pr-10 text-[13px] uppercase outline-none placeholder:text-text-muted focus:border-primary-500 min-[1600px]:h-[51px]" onChange={(event) => setQuery(event.target.value.trim().toUpperCase())} placeholder="搜索全体代币 · 代码 / 名称" value={query}/><kbd className="absolute right-3 top-1/2 -translate-y-1/2 rounded border border-border-subtle px-1.5 py-0.5 font-mono text-[11px] text-text-muted">/</kbd></div>
      <div className="mr-1.5 flex min-w-0 shrink-0 items-center gap-2 text-[11px] text-text-muted min-[1600px]:mr-3 max-[1279px]:hidden"><span className={`h-2 w-2 shrink-0 rounded-full ${error ? "bg-risk" : loading ? "animate-pulse bg-warn" : "bg-good"}`}/><span className="truncate">更新 {updatedAt}</span><span aria-hidden="true" className="h-8 w-8 shrink-0"/></div>
    </div>
    <div className="workstation-scroll min-h-0 flex-1 overflow-auto">
      <div className={`sticky top-0 z-10 grid h-[47px] min-w-[1028px] ${columns} items-center border-b border-border-subtle bg-[#f1f4f8] text-[14px] font-semibold min-[1600px]:h-[48px] min-[1600px]:min-w-0 [&>*]:min-w-0 [&>*]:px-3 [&>*]:!pr-4 min-[1600px]:[&>*]:!pr-7`}><span/><span className="text-right text-[#98a1ad]">币种</span>{header("净流入($)", "net_flow_usd")}{header("净流入变化", "net_flow_change_pct")}{header("交易量($)", "volume_usd")}{header("交易量变化", "volume_change_pct")}{header("流入($)", "inflow_usd")}{header("流出($)", "outflow_usd")}{header("市值($)", "market_cap")}{header("当前币价($)", "price")}{header("价格(24小时%)", "price_change_pct")}</div>
      {assets.map((item, index) => {
        ...
        return <div className={`grid h-[62px] min-w-[1028px] w-full cursor-pointer ${columns} items-center border-b border-[#fafafb] text-left font-mono text-[15px] font-semibold hover:bg-primary-50/45 min-[1600px]:h-[63.5px] min-[1600px]:min-w-0 [&>*]:min-w-0 [&>*]:px-3 [&>span]:truncate`} data-testid="funds-asset-row" key={symbol || index} onClick={() => onSelect(symbol)} onKeyDown={(event) => { if (event.key === "Enter") onSelect(symbol); }} role="button" tabIndex={0}>
          <button aria-label={`${favorite ? "取消" : "添加"}${item.coin || symbol}自选`} className={`h-full !pl-0 !pr-[9px] text-right text-[22px] font-normal min-[1600px]:!pr-[6px] ${favorite ? "text-warn" : "text-text-muted"}`} onClick={(event) => { event.stopPropagation(); onFavorite(symbol); }} type="button">{favorite ? "★" : "☆"}</button>
          <span className="flex items-center gap-3 !pl-6 min-[1600px]:!pl-7"><span className={`w-6 shrink-0 text-right font-bold ${rankTone}`}>{(page - 1) * pageSize + index + 1}</span><CoinIcon coin={item.coin} size={34}/><span className="ml-1 truncate text-text-primary">{item.coin || symbol}</span></span>
          <span className="flex h-full items-center !px-0"><span className={`relative ml-0.5 mr-px flex h-[29px] w-full -translate-y-px items-center justify-end overflow-hidden rounded-[2px] pr-[9px] min-[1600px]:-ml-1 min-[1600px]:mr-[6px] min-[1600px]:-translate-y-0.5 ${positiveFlow ? "text-good" : "text-risk"}`}><span aria-hidden="true" className={`absolute inset-y-0 right-0 ${positiveFlow ? "bg-[#daf1e8]" : "bg-[#fae4e6]"}`} style={{ width: `${netFlowBarWidth}%` }}/><span className="relative z-[1]">{cnMoney(item.net_flow_usd, false)}</span></span></span>
...
export default function FundsPage() {
  ...
  return <div aria-busy={loading || Boolean(selected && coinLoading)} className="workstation-page mercu-funds-grid" data-testid="funds-workstation">
    <section className="workstation-scroll flex h-[63px] max-w-full shrink-0 items-center gap-3 overflow-x-auto border-b border-border-subtle bg-surface-panel px-[18px] min-[1600px]:px-[25px]">
      <div className="flex h-12 rounded-[6px] border border-border-subtle bg-[#f1f4f8] p-[3px] min-[1600px]:h-[51px] min-[1600px]:w-[188px] min-[1600px]:translate-y-px" role="group">{(["spot", "futures"] as const).map((value) => <button aria-pressed={marketType === value} className={`h-10 min-w-[72px] flex-1 rounded-[4px] px-4 text-[14px] font-semibold min-[1600px]:h-[43px] ${marketType === value ? "bg-surface-panel text-text-primary shadow-sm ring-1 ring-border-subtle" : "text-text-muted"}`} onClick={() => { setAssetPage(1); setMarketType(value); }} type="button" key={value}>{value === "futures" ? "合约" : "现货"}</button>)}</div>
    ...
    {!selected ? <main className="mercu-funds-overview-grid grid min-h-0 flex-1 grid-cols-[350px_minmax(0,1fr)] gap-[18px] px-[18px] pb-[17px] pt-[7px] min-[1600px]:px-[25px] min-[1600px]:pb-[18px] min-[1600px]:pt-[13px]"><SectorOverview onWindow={setSectorWindow} payload={currentSectors} windowSec={sectorWindow}/><AssetsOverview assets={currentAssets} direction={sortDirection} error={error} favorites={favorites} loading={loading} onFavorite={toggleFavorite} onPage={setAssetPage} onSelect={setSelected} onSort={toggleSort} onWindow={(value) => { setAssetPage(1); setAssetWindow(value); }} payload={marketType === "spot" ? spotAssets : futuresAssets} query={query} setQuery={setQuery} sortKey={assetSort} windowSec={assetWindow}/></main> : <>
