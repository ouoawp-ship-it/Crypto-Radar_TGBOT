...
function SectorBubbleChart({ payload }: { payload: FundsSectorsPayload }) {
  ...
  const positivePositions = [[50, 18], [36, 39], [64, 38], [47, 47], [25, 43], [75, 43], [18, 56], [83, 56], [34, 58]];
  ...
  const bubble = (item: (typeof sectors)[number], index: number, count: number, top: boolean) => {
    const size = 28 + Math.sqrt(Math.abs(Number(item.net_flow_usd || 0)) / max) * 34;
...
export default function FundsPage() {
  ...
  return <div aria-busy={loading || Boolean(selected && coinLoading)} className="workstation-page mercu-funds-grid" data-testid="funds-workstation">
    <section className="workstation-scroll flex h-[44px] max-w-full shrink-0 items-center gap-2 overflow-x-auto border-b border-border-subtle px-2.5 min-[1024px]:px-4">
      <div className="flex rounded-[4px] border border-border-subtle bg-surface-low p-[2px]" role="group">{(["spot", "futures"] as const).map((value) => <button aria-pressed={marketType === value} className={`h-7 min-w-12 rounded-[3px] px-3 text-[9px] font-semibold min-[1024px]:h-8 min-[1024px]:min-w-14 ${marketType === value ? "bg-surface-panel text-text-primary shadow-sm ring-1 ring-border-subtle" : "text-text-muted"}`} onClick={() => { setAssetPage(1); setMarketType(value); }} type="button" key={value}>{value === "futures" ? "合约" : "现货"}</button>)}</div>
    ...
    {!selected ? <main className="grid min-h-0 flex-1 grid-cols-[225px_minmax(0,1fr)] gap-3 px-2.5 py-1.5 min-[1024px]:grid-cols-[minmax(225px,23.6%)_minmax(0,1fr)] min-[1024px]:px-4 min-[1024px]:py-2"><SectorOverview onWindow={setSectorWindow} payload={currentSectors} windowSec={sectorWindow}/><AssetsOverview assets={currentAssets} direction={sortDirection} error={error} favorites={favorites} loading={loading} onFavorite={toggleFavorite} onPage={setAssetPage} onRefresh={() => { void loadOverview(true); }} onSelect={setSelected} onSort={toggleSort} onWindow={(value) => { setAssetPage(1); setAssetWindow(value); }} payload={marketType === "spot" ? spotAssets : futuresAssets} query={query} setQuery={setQuery} sortKey={assetSort} windowSec={assetWindow}/></main> : <>
