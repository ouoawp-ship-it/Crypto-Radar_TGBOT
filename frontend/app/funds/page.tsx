...
function AssetsOverview({ assets, payload, query, setQuery, windowSec, onWindow, onSelect, onPage, onRefresh, loading, error, sortKey, direction, onSort, favorites, onFavorite }: {
  ...
  return <section className="workstation-panel flex min-h-0 min-w-0 flex-col" data-testid="funds-assets-overview">
    <div className="flex h-[38px] shrink-0 items-center gap-2 border-b border-border-subtle bg-surface-low px-2 min-[1024px]:h-[42px] min-[1280px]:h-[46px]">
      ...
      <div className="relative ml-[30px] w-[255px] shrink-0 min-[1280px]:ml-auto min-[1280px]:w-[280px]"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-[9px] text-text-muted">⌕</span><input aria-label="搜索全体代币" className="h-7 w-full rounded-[3px] border border-border-subtle bg-surface-panel pl-6 pr-7 text-[8px] uppercase outline-none placeholder:text-text-muted focus:border-primary-500" onChange={(event) => setQuery(event.target.value.trim().toUpperCase())} placeholder="搜索全体代币 · 代码 / 名称" value={query}/><kbd className="absolute right-2 top-1/2 -translate-y-1/2 rounded border border-border-subtle px-1 font-mono text-[7px] text-text-muted">/</kbd></div>
      <div className="ml-auto flex min-w-0 items-center gap-1.5 text-[7px] text-text-muted min-[1280px]:hidden"><span className={`h-1.5 w-1.5 shrink-0 rounded-full ${error ? "bg-risk" : loading ? "animate-pulse bg-warn" : "bg-good"}`}/><span className="truncate">更新 {updatedAt}</span><button aria-label="刷新资金榜" className="grid h-5 w-5 shrink-0 place-items-center rounded-[2px] border border-border-subtle" disabled={loading} onClick={onRefresh} type="button">↻</button></div>
    </div>
