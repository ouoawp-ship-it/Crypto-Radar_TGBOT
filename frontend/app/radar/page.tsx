...
function EventFeed({ events, query }: { events: RealtimeAnomalyEvent[]; query: string }) {
  ...
  return <div className="workstation-scroll min-h-0 flex-1 overflow-y-auto">
    {filtered.map((event, index) => {
      ...
      return <Link className="block border-b border-border-subtle px-2.5 py-[7px] transition-colors hover:bg-primary-50/55 min-[1024px]:py-[2px] min-[1280px]:py-[1px]" href={`/funds?symbol=${event.symbol || ""}`} key={event.id || `${event.symbol}-${event.event_type}-${index}`}>
...
function MomentumStrengthGrid({ items, positive, realtimeBySymbol }: { items?: CockpitBoardItem[]; positive: boolean; realtimeBySymbol: Map<string, RealtimeIntelligenceItem> }) {
  return <div className="grid grid-cols-1 sm:grid-cols-2" data-testid="radar-strength-grid">{(items || []).slice(0, 8).map((item, index) => {
    ...
    return <Link className="flex h-[34px] min-w-0 flex-col items-center justify-center border-b border-r border-border-subtle/70 px-0.5 hover:bg-primary-50/55 min-[1024px]:h-[42px] min-[1280px]:h-[40px]" href={`/funds?symbol=${item.symbol || ""}`} key={`${item.symbol}-${index}`}>
...
function MomentumBoard({ board, realtimeBySymbol }: { board?: CockpitBoard; realtimeBySymbol: Map<string, RealtimeIntelligenceItem> }) {
  ...
  return <section className="overflow-hidden rounded-[2px] border border-border-subtle bg-surface-panel">
    ...
    <div className="grid grid-cols-2 divide-x divide-border-subtle"><MomentumList items={amountPositivם��G����ƭy�</span><span className="rounded-[2px] bg-warn/10 px-1 text-[7px] text-warn">强度榜</span></div><div className="flex items-center justify-between px-2 text-risk"><span>▼ {labels.negative}</span><span className="rounded-[2px] bg-warn/10 px-1 text-[7px] text-warn">强度榜</span></div></div>
