...
function FollowWindowBadge({ windowKey }: { windowKey: WindowKey }) {
  return <span className="mr-[6px] inline-flex items-center gap-1.5 rounded-full border border-[#8da7e8] bg-[#e2e7f4] px-4 py-1 text-[12px] font-semibold leading-[15px] text-[#002fa7] min-[1280px]:mr-0 min-[1280px]:px-[18px] min-[1280px]:leading-normal"><span className="h-2 w-2 rounded-full bg-[#002fa7]"/>跟随 {windowKey}</span>;
}
...
function MomentumList({ items, mode, onSelectSymbol, positive, realtimeBySymbol, scaleMax, windowStates, compactMillions = false, limit = 7 }: { items?: CockpitBoardItem[]; mode: RankMode; onSelectSymbol: (symbol: string) => void; positive: boolean; realtimeBySymbol: Map<string, RealtimeIntelligenceItem>; scaleMax?: number; windowStates: (item: CockpitBoardItem) => boolean[] | undefined; compactMillions?: boolean; limit?: number }) {
  ...
  return <div>{visible.map((item, index) => {
    ...
    return <button className="relative grid h-[29px] w-full grid-cols-[20px_20px_minmax(0,1fr)_48px_58px] items-center gap-1 overflow-hidden px-3 text-left text-[11px] hover:bg-primary-50/50 min-[1280px]:h-[30px]" data-symbol={item.symbol || ""} key={`${item.symbol}-${index}`} onClick={() => onSelectSymbol(String(item.symbol || item.coin || ""))} type="button">
...
function MomentumStrengthGrid({ items, onSelectSymbol, positive, realtimeBySymbol, windowStates, compactMillions = false }: { items?: CockpitBoardItem[]; onSelectSymbol: (symbol: string) => void; positive: boolean; realtimeBySymbol: Map<string, RealtimeIntelligenceItem>; windowStates: (item: CockpitBoardItem) => boolean[] | undefined; compactMillions?: boolean }) {
  return <div className="grid grid-cols-1 sm:grid-cols-2 [&>:nth-last-child(-n+2)]:border-b-0" data-testid="radar-strength-grid">{(items || []).slice(0, 8).map((item, index) => {
    ...
    const active = states.filter(Boolean).length;
    return <button className="grid h-[52px] min-w-0 grid-cols-[12px_18px_minmax(0,1fr)_38px] grid-rows-[14px_14px] content-center items-center gap-x-1 border-b border-r border-border-subtle/70 border-b-border-subtle/40 px-3 text-left hover:bg-primary-50/55 min-[1280px]:pt-[6px]" data-symbol={item.symbol || ""} key={`${item.symbol}-${index}`} onClick={() => onSelectSymbol(String(item.symbol || item.coin || ""))} type="button">
...
function MomentumBoard({ board, momentum, onSelectSymbol, realtimeBySymbol }: { board?: CockpitBoard; momentum: Partial<Record<WindowKey, RadarBoards>>; onSelectSymbol: (symbol: string) => void; realtimeBySymbol: Map<string, RealtimeIntelligenceItem> }) {
  ...
  return <section className="overflow-hidden rounded-[2px] border border-border-subtle bg-surface-panel">
    <div className="grid h-[37px] grid-cols-2 border-b border-border-subtle bg-surface-panel text-[12px] font-bold min-[1280px]:h-[38px]"><div className="flex items-center justify-between border-r border-border-subtle px-3 max-[1279px]:min-w-0"><span className="flex items-center gap-1.5 max-[1279px]:min-w-0 max-[1279px]:flex-1 max-[1279px]:overflow-hidden"><span className="max-[1279px]:min-w-0 max-[1279px]:truncate max-[1279px]:whitespace-nowrap"><b className="mr-1.5 text-good">▲</b>{labels.positive}</span><span className="rounded-[3px] bg-surface-container px-1.5 py-0.5 text-[10px] text-text-muted max-[1279px]:shrink-0">量级榜</span></span><small className="font-mono text-[9px] font-normal text-text-muted/60 max-[1279px]:ml-1 max-[1279px]:shrink-0">{amountUnit}</small></div><div className="flex items-center justify-between px-3 max-[1279px]:min-w-0"><span className="flex items-center gap-1.5 max-[1279px]:min-w-0 max-[1279px]:flex-1 max-[1279px]:overflow-hidden"><span className="max-[1279px]:min-w-0 max-[1279px]:truncate max-[1279px]:whitespace-nowrap"><b className="mr-1.5 text-risk">▼</b>{labels.negative}</span><span className="rounded-[3px] bg-surface-container px-1.5 py-0.5 text-[10px] text-text-muted max-[1279px]:shrink-0">量级榜</span></span><small className="font-mono text-[9px] font-normal text-text-muted/60 max-[1279px]:ml-1 max-[1279px]:shrink-0">{amountUnit}</small></div></div>
    ...
    <div className="grid h-[40px] grid-cols-2 border-y border-border-subtle bg-surface-panel text-[12px] font-bold min-[1280px]:h-[39px]"><div className="flex items-center justify-between border-r border-border-subtle px-3 max-[1279px]:min-w-0"><span className="flex items-center gap-1.5 max-[1279px]:min-w-0 max-[1279px]:flex-1 max-[1279px]:overflow-hidden"><span className="max-[1279px]:min-w-0 max-[1279px]:truncate max-[1279px]:whitespace-nowrap"><b className="mr-1.5 text-good">▲</b>{labels.positive}</span><span className="rounded-[3px] bg-warn/10 px-1.5 py-0.5 text-[10px] text-warn max-[1279px]:shrink-0">强度榜</span></span><small className="text-[9px] font-normal text-text-muted/60 max-[1279px]:ml-1 max-[1279px]:shrink-0">强度分</small></div><div className="flex items-center justify-between px-3 max-[1279px]:min-w-0"><span className="flex items-center gap-1.5 max-[1279px]:min-w-0 max-[1279px]:flex-1 max-[1279px]:overflow-hidden"><span className="max-[1279px]:min-w-0 max-[1279px]:truncate max-[1279px]:whitespace-nowrap"><b className="mr-1.5 text-risk">▼</b>{labels.negative}</span><span className="rounded-[3px] bg-warn/10 px-1.5 py-0.5 text-[10px] text-warn max-[1279px]:shrink-0">强度榜</span></span><small className="text-[9px] font-normal text-text-muted/60 max-[1279px]:ml-1 max-[1279px]:shrink-0">强度分</small></div></div>
...
export default function RadarPage() {
  ...
  return <><div aria-busy={loading} className="workstation-page mercu-radar-grid" data-testid="radar-workstation">
    <aside className="workstation-panel mobile-radar-event-feed flex min-h-0 flex-col" data-testid="radar-event-feed">
      ...
      <div className="radar-scan-bar grid h-[60px] grid-cols-[minmax(0,1fr)_128px] items-center gap-2 border-b border-border-subtle bg-[#f1f4f8] pl-[18px] pr-0 min-[1280px]:grid-cols-[minmax(0,1fr)_168px] min-[1280px]:pr-3"><span className="flex min-w-0 items-center gap-[14px] text-[14px] font-semibold text-primary-700"><i aria-hidden="true" className="radar-scan-orbit" data-testid="radar-scan-orbit"><span/></i><span className="flex min-w-0 flex-col"><span className="flex items-center gap-1.5 truncate leading-5"><i aria-hidden="true" className="hidden h-1.5 w-1.5 shrink-0 rounded-full bg-good/60 min-[1280px]:block"/>全市场实时扫描</span><small className="font-mono text-[10px] font-normal leading-4 text-text-muted">1000+ 币种</small></span></span><div className="relative"><svg aria-hidden="true" className="absolute left-3 top-1/2 h-[15px] w-[15px] -translate-y-1/2 text-text-muted" fill="none" viewBox="0 0 16 16"><circle cx="7" cy="7" r="4.5" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5"/><path d="m10.5 10.5 3 3" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5"/></svg><input aria-label="搜索币种" className="h-[34px] w-full rounded-[4px] border border-border-subtle bg-surface-panel pl-8 pr-2 text-[11px] uppercase text-text-primary outline-none placeholder:text-text-muted focus:border-primary-500" onChange={(event) => setQuery(event.target.value.trim().toUpperCase())} placeholder="搜索币种..." value={query}/></div></div>
    ...
    <main className="workstation-scroll min-h-0 overflow-y-auto" data-testid="radar-hot-money">
      <section className="workstation-panel flex min-h-[610px] flex-col max-[767px]:min-h-0">
        <PanelTitle action={<div className="flex h-[36px] items-center overflow-hidden rounded-[6px] border border-[#cbd2dc] bg-[#e9edf3]">{WINDOWS.map((key) => <button aria-pressed={windowKey === key} className={`h-[28px] w-[46px] min-w-0 rounded-[4px] px-0 font-mono text-[13px] font-semibold max-[767px]:w-11 min-[1280px]:w-[60px] ${windowKey === key ? "bg-[#d1daeb] text-[#002fa7] ring-1 ring-[#738dcc]" : "text-text-muted hover:bg-[#f8f8f9] hover:text-text-primary"}`} key={key} onClick={() => setWindowKey(key)} type="button">{key}</button>)}</div>} icon="◎" meta={`更新 ${clock(momentum[windowKey]?.generated_at)}`} tip={RADAR_TIPS.hotMoney} title="热钱观察榜单"/>
        <div className="grid min-h-0 flex-1 auto-rows-[489px] grid-cols-1 content-start gap-x-[18px] gap-y-[17px] overflow-visible px-2 pb-[11px] pt-3 min-[768px]:grid-cols-2 min-[768px]:pl-3 min-[768px]:pr-[18px] min-[1280px]:auto-rows-[500px] min-[1280px]:gap-y-[18px] min-[1280px]:pt-[11px]" data-testid="radar-momentum-matrix">{["price", "oi", "futures_flow", "spot_flow"].map((key) => <MomentumBoard board={boards.find((board) => board.key === key)} key={key} momentum={momentum} onSelectSymbol={setSelectedCoin} realtimeBySymbol={realtimeBySymbol}/>)}</div>
      </section>
      <div className="radar-paoxx-extension mt-1.5 grid h-[220px] min-h-0 grid-cols-[.9fr_1.15fr_.95fr] gap-1.5 max-[767px]:h-auto max-[767px]:grid-cols-1" data-testid="radar-paoxx-extension"><RuleBoard items={surge} mode="surge" onSelectSymbol={setSelectedCoin} subtitle="1h 滚动 · 加速度排序 · TOP 5" title="Surge 飙升榜"/><RuleBoard items={total} mode="total" onSelectSymbol={setSelectedCoin} subtitle="24h 累计异动 · TOP 14" title="24h 异动总榜"/><RuleBoard items={ambush} mode="ambush" onSelectSymbol={setSelectedCoin} subtitle="持仓蓄积 / 价格平静 / 等待突破" title="埋伏池"/></div>
    </main>

    <aside className="workstation-scroll min-h-0 overflow-y-auto" data-testid="radar-side-intelligence">
      <section className="workstation-panel h-[669px] min-[1280px]:h-[679px]"><PanelTitle action={<FollowWindowBadge windowKey={windowKey}/>} icon="↝" iconClassName="text-warn" title="资金倾向性"/>
        <div className="flex h-[37px] items-center gap-2 bg-surface-panel px-4 text-[13px] font-bold text-text-primary min-[1280px]:h-[38px]"><span aria-hidden="true" className="h-[14px] w-[3px] rounded-full bg-warn/60"/>资金合流</div>
        ...
        <div className="flex h-[38px] items-center gap-2 border-y border-border-subtle bg-surface-panel px-4 text-[13px] font-bold text-text-primary min-[1280px]:h-[45px]"><span aria-hidden="true" className="h-[14px] w-[3px] rounded-full bg-warn/60"/>资金力度</div>
