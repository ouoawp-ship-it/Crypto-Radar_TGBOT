          ...
          <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_320px]">
            ...
            <aside className="space-y-3">
              <section className="cockpit-panel">
                ...
                {[["市场快照", data.evidence_coverage?.market], ["K 线样本", data.evidence_coverage?.chart_points], ["历史快照", data.evidence_coverage?.snapshot_points], ["历史信号", data.evidence_coverage?.signals], ["关联公告", data.evidence_coverage?.announcements]].map(([label, value]) => <div className="flex items-center justify-between border-b border-border-subtle px-3 py-2.5 last:border-0" key={String(label)}><span className="text-xs text-text-secondary">{label}</span><span className="table-number text-xs font-semibold text-text-primary">{Number(value || 0)}</span></div>)}
              </section>
              ...
              <section className="cockpit-panel">
                <div className="cockpit-panel-header"><div><h2 className="text-sm font-semibold text-text-primary">关联公告</h2><p className="mt-0.5 text-[11px] text-text-muted">仅展示已入库的官方事件</p></div></div>
                {(data.related_info?.items || []).length ? (data.related_info?.items || []).map((item) => <Link className="block border-b border-border-subtle px-3 py-2.5 last:border-0 hover:bg-surface-container-low" href={`/radar?symbol=${symbol}&signal=${item.public_ref || item.id}`} key={item.public_ref || item.id}><div className="text-xs font-semibold text-text-primary">{safeText(item.display?.title, item.excerpt)}</div><div className="mt-1 text-[10px] text-text-muted">{formatDateTime(item.time)}</div></Link>) : <div className="px-3 py-8 text-center text-xs text-text-muted">暂无关联官方公告</div>}
              </section>
