...
import { getCoinContext, getWorkstationFundsOpenInterest } from "@/lib/api";
import { formatDateTime, formatMetricValue, safeText } from "@/lib/format";
import type { CoinContext, CrossExchangeOpenInterest, RealtimeAnomalyEvent } from "@/lib/types";
...
const FLOW_WINDOWS = ["5m", "15m", "30m", "1h", "4h", "1d"] as const;
...
function RelatedInfo({ data }: { data: CoinContext | null }) {
  const items = data?.related_info?.items || data?.timeline || [];
  return <section className="border-t border-border-subtle pt-2">
    <h2 className="text-[10px] font-bold text-text-primary">相关信息 <span className="font-normal text-text-muted">· 最近 24h</span></h2>
    <div className="mt-1 overflow-hidden rounded-[2px] border border-border-subtle">
      {items.length ? items.slice(0, 8).map((item, index) => <article className="border-b border-border-subtle/70 bg-[#f7f9fc] px-2.5 py-2 last:border-0" key={item.public_ref || item.id || index}>
        <div className="flex items-center gap-1 text-[7px]"><span className="rounded-[2px] bg-warn/10 px-1 font-semibold text-warn">{safeText(item.display?.module_label, item.module || "情报")}</span><time className="ml-auto font-mono text-text-muted">{formatDateTime(item.time).slice(-5)}</time></div>
        <p className="mt-1 text-[8px] leading-4 text-text-secondary">{safeText(item.display?.summary, item.excerpt || item.display?.title)}</p>
      </article>) : <div className="grid h-16 place-items-center text-[8px] text-text-muted">最近 24h 暂无相关信息</div>}
    </div>
  </section>;
}
...
export function MercuCoinDrawer({ events, onClose, symbol }: { events: RealtimeAnomalyEvent[]; onClose: () => void; symbol: string }) {
  ...
  const [flowWindow, setFlowWindow] = useState<(typeof FLOW_WINDOWS)[number]>("1h");
      ...
      <div className="grid min-h-0 flex-1 overflow-hidden min-[760px]:grid-cols-[38%_62%]">
        ...
        <div className="workstation-scroll min-h-0 overflow-y-auto">
          ...
          <section className="border-b border-border-subtle px-3 py-2.5">
            <div className="flex items-center gap-1"><h2 className="text-[10px] font-bold text-text-primary">资金流</h2><span className="text-[8px] text-text-muted">· 过去 5 天</span><div className="ml-auto flex gap-1">{FLOW_WINDOWS.map((item) => <button aria-pressed={flowWindow === item} className={`h-5 min-w-7 rounded-[2px] px-1.5 font-mono text-[7px] ${flowWindow === item ? "bg-primary-50 text-primary-700 ring-1 ring-primary-500/25" : "text-text-muted hover:bg-surface-low"}`} key={item} onClick={() => setFlowWindow(item)} type="button">{item}</button>)}</div></div>
