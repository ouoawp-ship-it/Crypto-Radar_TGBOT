...
def flow_category(item: dict[str, Any]) -> tuple[str, int, str]:
    ...
    short_fuel += 20 if futures < 0 else 0
    ...
    perp_pump += 20 if not spot_positive else 0
    ...
    short_squeeze += 10 if not spot_positive else 0
    ...
    distribution += 30 if not spot_positive else 0
    ...
    panic += 25 if not spot_positive else 0
    panic += 15 if not futures_positive else 0
...
class FlowRadarEngine:
    ...
    def build(self, binance: BinanceDataSource, coinglass: CoinglassDataSource) -> dict[str, Any]:
        ...
        for candidate in candidates[: max(1, self.settings.flow_scan_limit)]:
            ...
            spot_cvd = series_delta(coinglass.spot_aggregated_cvd_history(
                coin,
                exchange_list=self.settings.coinglass_exchange_list,
                interval="1h",
                limit=6,
            ))
            futures_cvd = series_delta(coinglass.futures_aggregated_cvd_history(
                coin,
                exchange_list=self.settings.coinglass_exchange_list,
                interval="1h",
                limit=6,
            ))
            price_24h = to_float(market.get("price_change_percent_24h"), candidate["price_24h"])
            oi_24h = to_float(market.get("open_interest_change_percent_24h"), 0.0)
            funding_pct = normalise_pct(to_float(market.get("avg_funding_rate_by_oi"), candidate.get("funding_pct", 0.0)))
            quote_volume = to_float(
                market.get("volume_change_usd_24h")
                or market.get("volume_usd")
                or candidate["quote_volume"]
            )
            oi_usd = to_float(market.get("open_interest_usd"), 0.0)
            item = {
                "symbol": symbol,
                "coin": coin,
                "price_24h": price_24h,
        ...
        return {
            "template_id": "TG_FLOW_RADAR",
            "dedup_key": f"flow-radar:{datetime.now(CST).strftime('%Y%m%d%H%M')}",
            "text": self._format(rows, candidates, coinglass),
            "items": rows,
            "diagnostics": {"coinglass": coinglass.diagnostics(), "binance": binance.diagnostics()},
        }
    ...
    def _format(self, rows: list[dict[str, Any]], candidates: list[dict[str, Any]], coinglass: CoinglassDataSource) -> str:
        lines = [
            "🧭 <b>五因子资金流雷达</b>",
            f"⏰ {cst_now_text()}",
            "",
        ...
        for category in ("真启动候选", "吸筹观察", "空头燃料", "合约拉盘", "挤空/止损", "诱多/派发", "恐慌下跌"):
            ...
            for item in items[:4]:
                ...
                lines.append(
                    ...
                    f"现货CVD {fmt_money(item['spot_cvd_delta'])} | "
                    f"合约CVD {fmt_money(item['futures_cvd_delta'])} | "
