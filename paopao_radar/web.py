...
<head>
  ...
  <style>
    :root{--bg:#f4f7fb;--panel:#fff;--panel-2:#f8fafc;--text:#111827;--muted:#667085;--line:#dbe3ef;--accent:#206bc4;--good:#2fb344;--warn:#f59f00;--bad:#d63939;--info:#206bc4;--shadow:0 10px 28px rgba(17,24,39,.07)}
    *{box-sizing:border-box} body{margin:0;background:linear-gradient(180deg,#f8fbff,#eef3f8);color:var(--text);font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    .wrap{max-width:1220px;margin:0 auto;padding:18px}.top{display:flex;justify-content:space-between;gap:16px;align-items:center}.brand h1{margin:0;font-size:28px}.brand p{margin:4px 0 0;color:var(--muted)}
    .admin-link,.tab,.btn{border:1px solid var(--line);border-radius:8px;padding:9px 12px;color:var(--accent);text-decoration:none;background:#fff;font-weight:800;cursor:pointer}.btn.primary,.tab.active{background:var(--accent);border-color:var(--accent);color:#fff}
    .tabs{display:flex;gap:8px;flex-wrap:wrap;margin:16px 0}.grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:14px}.panel{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:14px;box-shadow:var(--shadow)}
    .span-12{grid-column:span 12}.span-8{grid-column:span 8}.span-7{grid-column:span 7}.span-5{grid-column:span 5}.span-4{grid-column:span 4}.span-3{grid-column:span 3}
    .metric strong{display:block;font-size:24px}.metric span,.muted{color:var(--muted);font-size:13px}.toolbar{display:grid;grid-template-columns:1.4fr 1fr 1fr 1fr 1fr auto auto;gap:8px;align-items:center}
    input,select,button{border:1px solid var(--line);border-radius:8px;padding:9px 10px;background:#fff;color:var(--text);min-width:0}button{font-weight:800;cursor:pointer}.signal-card-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
    .signal-card{border:1px solid var(--line);border-left:4px solid var(--accent);border-radius:10px;padding:13px;background:#fff;display:grid;gap:9px;transition:.15s ease}.signal-card:hover{border-color:#adc3dd;box-shadow:0 12px 26px rgba(17,24,39,.09);transform:translateY(-1px)}
    .signal-card.good{border-left-color:var(--good)}.signal-card.warn{border-left-color:var(--warn)}.signal-card.bad{border-left-color:var(--bad)}.signal-card.info{border-left-color:var(--info)}.signal-card.neutral{border-left-color:#94a3b8}
    .head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.title{font-weight:900;line-height:1.25}.summary{color:#344054;font-size:14px;line-height:1.48}.meta{display:flex;gap:8px;flex-wrap:wrap;align-items:center;color:var(--muted);font-size:12px}
    .badge{display:inline-flex;border:1px solid var(--line);border-radius:999px;padding:3px 8px;font-size:12px;font-weight:850;background:#f8fafc}.badge.clickable{cursor:pointer}.good{color:var(--good);background:#edf9f0}.warn{color:#a16207;background:#fff7df}.bad{color:var(--bad);background:#fff0f0}.info{color:var(--info);background:#edf5ff}.neutral{color:#64748b;background:#f8fafc}
    .timeline-day{border-left:2px solid #dbe6f3;padding-left:12px;display:grid;gap:8px}.timeline-day h3{margin:0;color:var(--muted);font-size:13px}.timeline-item{border:1px solid var(--line);border-radius:8px;padding:10px;background:#fff;display:grid;gap:6px}.coin-actions{display:flex;gap:8px;flex-wrap:wrap}.coin-card{display:grid;gap:8px;border:1px solid var(--line);border-radius:9px;padding:10px;background:#fff}
    .empty,.loading,.error{border:1px dashed var(--line);border-radius:10px;padding:18px;color:var(--muted);background:#f8fafc}.error{color:var(--bad);background:#fff7f7}.modal-backdrop{position:fixed;inset:0;background:rgba(15,23,42,.46);display:none;align-items:flex-start;justify-content:center;padding:48px 16px;z-index:30}.modal-backdrop.open{display:flex}.modal{width:min(760px,100%);max-height:88vh;overflow:auto;background:#fff;border-radius:12px;border:1px solid var(--line);box-shadow:0 24px 60px rgba(15,23,42,.25);padding:16px}.modal pre{white-space:pre-wrap;word-break:break-word;background:#f8fafc;border:1px solid var(--line);border-radius:8px;padding:10px;max-height:280px;overflow:auto}.hidden{display:none!important}
    @media(max-width:940px){.span-8,.span-7,.span-5,.span-4,.span-3{grid-column:span 12}.toolbar{grid-template-columns:1fr 1fr}.signal-card-grid{grid-template-columns:1fr}.top{align-items:flex-start;flex-direction:column}}
    @media(max-width:560px){.wrap{padding:12px}.toolbar{grid-template-columns:1fr}.tabs{position:sticky;top:74px;z-index:8;background:rgba(244,247,251,.94);padding:8px 0}.brand h1{font-size:22px}.panel{padding:12px}.modal-backdrop{padding:18px 8px}.admin-link,.tab,.btn{width:100%;text-align:center}}
  </style>
</head>
<body>
  ...
  <main class="wrap">
    <div class="tabs"><button class="tab active" data-view="home">Signals</button><button class="tab" data-view="timeline">Market Timeline</button><button class="tab" data-view="coin">Coin Detail</button></div>
    <section class="grid">
      <div class="panel span-12"><div class="toolbar" id="publicFilters">
        <input id="filterQ" placeholder="Keyword">
        <input id="filterSymbol" placeholder="BTC / BTCUSDT">
        <select id="filterModule"><option value="">All modules</option><option value="launch">Launch</option><option value="funding">Funding</option><option value="flow">Flow</option><option value="structure">Structure</option><option value="structure_review">Review</option><option value="announcement">Announcement</option><option value="summary">Summary</option><option value="test">Test</option></select>
        <select id="filterStatus"><option value="">All status</option><option value="sent">Sent</option><option value="dry_run">Dry-run</option><option value="skipped">Skipped</option><option value="blocked">Blocked</option><option value="failed">Failed</option></select>
        <select id="filterWindow"><option value="86400">24h</option><option value="604800" selected>7d</option><option value="2592000">30d</option></select>
        <button class="btn primary" id="applyFilters">Refresh</button><button class="btn" id="clearFilters">Clear</button>
      </div></div>
      <div id="stats" class="span-12 grid"></div>
      <div id="homeView" class="span-12 grid"><div class="panel span-8"><div class="head"><h2>Latest Signal Cards</h2><span class="muted" id="signalCount"></span></div><div id="signals" class="signal-card-grid"></div></div><div class="panel span-4"><h2>Active Coins</h2><div id="coins" class="cards"></div></div></div>
      <div id="timelineView" class="span-12 panel hidden"><div class="head"><h2>All-Market Timeline</h2><span class="muted" id="timelineCount"></span></div><div id="timeline" class="cards"></div></div>
      <div id="coinView" class="span-12 grid hidden"><div class="panel span-5"><h2>Coin Detail</h2><div id="coinDetail" class="empty">Click a coin badge or search BTC / BTCUSDT.</div></div><div class="panel span-7"><h2>Coin Timeline</h2><div id="coinTimeline" class="cards"></div></div></div>
    </section>
  </main>
  <div id="signalDetailModal" class="modal-backdrop" role="dialog" aria-modal="true"><div class="modal"><div class="head"><h2 id="modalTitle">Signal Detail</h2><button class="btn" id="closeModal">Close</button></div><div id="modalBody" class="empty">Loading...</div></div></div>
  <script>
    const $ = id => document.getElementById(id);
    const state = {view:"home", loading:false, selectedCoin:""};
    ...
    function setView(view){ state.view=view; document.querySelectorAll(".tab").forEach(t=>t.classList.toggle("active",t.dataset.view===view)); ["home","timeline","coin"].forEach(name=>$(name+"View").classList.toggle("hidden",name!==view)); loadPublic().catch(showError); }
    ...
    function signalCard(item){ const d=item.display||{}; const itemTone=tone(item.status); const symbol=item.symbol||""; return `<article class="signal-card ${itemTone}" data-signal-id="${Number(item.id||0)}"><div class="head"><div><div class="title">${esc(d.title||item.signal_type||"Signal")}</div><div class="muted">${esc(d.time_label||item.time||"-")}</div></div>${badge(d.status_label||item.status,itemTone,`data-filter-status="${esc(item.status||"")}"`)}</div><div class="summary">${esc(d.summary||item.excerpt||"")}</div><div class="meta">${badge(d.module_label||item.module,"info",`data-filter-module="${esc(item.module||"")}"`)}${symbol?badge(symbol,"neutral",`data-coin="${esc(symbol)}"`):badge("Global","neutral")}<span>score ${esc(d.score_label ?? item.score ?? "-")}</span><span>stage ${esc(d.stage_label||item.stage||"-")}</span></div><div class="coin-actions"><button class="btn" data-signal-id="${Number(item.id||0)}">Details</button>${symbol?`<button class="btn" data-open-coin="${esc(symbol)}">Coin Detail</button>`:""}</div></article>`; }
    function statsHtml(stats){ const blockedFailed=Number(stats.blocked||0)+Number(stats.failed||0); return [["24h Signals",stats.total||0],["Sent",stats.sent||0],["Dry-run",stats.dry_run||0],["Blocked/Failed",blockedFailed]].map(([k,v])=>`<div class="panel span-3 metric"><span>${esc(k)}</span><strong>${esc(v)}</strong></div>`).join(""); }
    function timelineHtml(groups){ if(!groups||!groups.length) return `<div class="empty">No timeline events. Try a wider time window or fewer filters.</div>`; return groups.map(g=>`<section class="timeline-day"><h3>${esc(g.label||g.date)} / ${esc(g.count||0)} events</h3>${(g.items||[]).map(item=>`<div class="timeline-item">${signalCard(item)}</div>`).join("")}</section>`).join(""); }
    function coinListHtml(items){ if(!items||!items.length) return `<div class="empty">No active coins.</div>`; return items.map(c=>`<div class="coin-card"><div class="head"><b>${esc(c.label||c.symbol)}</b>${badge(`${c.count||0} signals`,"info")}</div><div class="muted">${esc(c.subtitle||"")}</div><button class="btn" data-open-coin="${esc(c.symbol||"")}">Open coin detail</button></div>`).join(""); }
    async function openSignalDetail(id){ $("signalDetailModal").classList.add("open"); $("modalBody").innerHTML=`<div class="loading">Loading signal detail...</div>`; const data=await api(`/public-api/signals/detail?id=${encodeURIComponent(id)}`); const detail=data.detail||{}; const header=detail.header||{}; $("modalTitle").textContent=header.title||"Signal Detail"; $("modalBody").innerHTML=((detail.sections||[]).map(section=>`<section><h3>${esc(section.title||"")}</h3>${(section.rows||[]).map(row=>`<p><b>${esc(row.label)}:</b> ${esc(row.value||"-")}</p>`).join("")}</section>`).join("") || `<div class="empty">No public detail.</div>`) + relatedHtml(detail.related); }
    function relatedHtml(related){ const items=((related||{}).same_symbol||[]).slice(0,5); if(!items.length) return ""; return `<section><h3>Related signals</h3><div class="cards">${items.map(signalCard).join("")}</div></section>`; }
    async function openCoin(symbol){ state.selectedCoin=symbol||$("filterSymbol").value.trim(); if(!state.selectedCoin) return; $("filterSymbol").value=state.selectedCoin; setView("coin"); const params=filterParams(50); params.set("symbol",state.selectedCoin); const data=await api(`/public-api/coin-detail?${params.toString()}`); const s=data.summary||{}; $("coinDetail").innerHTML=`<div class="coin-card"><h3>${esc(data.symbol||state.selectedCoin)}</h3><p>${esc(s.headline||"No summary yet.")}</p><div class="meta">${badge(s.health_label||s.health||"ok",s.health==="risk"?"bad":s.health==="attention"?"warn":"good")}${badge(`${s.total||0} signals`,"info")}${badge(`${s.active_modules||0} modules`,"neutral")}</div></div>${distributionHtml("Modules",data.module_counts||[])}${distributionHtml("Status",data.status_counts||[])}`; $("coinTimeline").innerHTML=timelineHtml(data.timeline_groups||data.timeline||[]); }
    ...
    function showError(err){ const msg=esc(err.message||String(err)); const panel=`<div class="error">${msg}<br><button class="btn" onclick="loadPublic()">Retry</button></div>`; if(state.view==="timeline") $("timeline").innerHTML=panel; else $("signals").innerHTML=panel; }
    async function loadPublic(){ if(state.loading) return; state.loading=true; try{ $("stats").innerHTML=statsHtml(await api(`/public-api/signals/stats?window_sec=86400`)); const [signals, coins, timeline]=await Promise.all([api(`/public-api/signals?${filterParams(40)}`),api(`/public-api/coin-search?limit=12&window_sec=${encodeURIComponent($("filterWindow").value||"604800")}`),api(`/public-api/signal-timeline?${filterParams(100)}`)]); $("signalCount").textContent=`${signals.count||0} shown`; $("signals").innerHTML=(signals.items||[]).length?(signals.items||[]).map(signalCard).join(""):`<div class="empty">No public signals match the filters.</div>`; $("coins").innerHTML=coinListHtml(coins.items||[]); $("timelineCount").textContent=`${timeline.count||0} events`; $("timeline").innerHTML=timelineHtml(timeline.groups||[]); } finally { state.loading=false; } }
