...
<body>
  ...
  <main class="wrap">
    <div class="tabs"><button class="tab active" data-view="home">首页</button><button class="tab" data-view="timeline">信号时间线</button><button class="tab" data-view="coin">币种详情</button></div>
    <section class="grid">
  ...
  <script>
    ...
    async function openCoin(symbol){ state.selectedCoin=symbol||$("filterSymbol").value.trim(); if(!state.selectedCoin) return; $("filterSymbol").value=state.selectedCoin; setView("coin"); const params=filterParams(50); params.set("symbol",state.selectedCoin); const data=await api(`/public-api/coin-detail?${params.toString()}`); const s=data.summary||{}; $("coinDetail").innerHTML=`<div class="coin-card"><h3>${esc(data.symbol||state.selectedCoin)}</h3><p>${esc(s.headline||"暂无摘要。")}</p><div class="meta">${badge(s.health_label||s.health||"正常",s.health==="risk"?"bad":s.health==="attention"?"warn":"good")}${badge(`${s.total||0} 条信号`,"info")}${badge(`${s.active_modules||0} 个模块`,"neutral")}</div></div>${distributionHtml("模块分布",data.module_counts||[])}${distributionHtml("状态分布",data.status_counts||[])}`; $("coinTimeline").innerHTML=timelineHtml(data.timeline_groups||data.timeline||[]); }
