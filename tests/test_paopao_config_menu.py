...
class ConfigManagerTests(unittest.TestCase):
    ...
    def test_comments_and_unknown_fields_are_preserved(self) -> None:
        ...
        self.manager.set("COINGLASS_API_KEY", "fake-secret")
        text = path.read_text(encoding="utf-8")
        self.assertIn("# keep this comment", text)
        self.assertIn("CUSTOM_KEEP=1", text)
        self.assertIn("COINGLASS_API_KEY=fake-secret", text)

    def test_secret_status_does_not_echo_value(self) -> None:
        secret = "fake-secret-for-test"
        result = self.manager.set("COINGLASS_API_KEY", secret)
        self.assertEqual(result["value"], "configured")
        self.assertNotIn(secret, str(result))
        self.assertEqual(
            self.manager.status()["COINGLASS_API_KEY"], "configured"
        )
    ...
    def test_tty_input_uses_visible_input(self) -> None:
        with patch("sys.stdin.isatty", return_value=True), patch(
            ...
            self.assertEqual(
                _read_value("COINGLASS_API_KEY"), "visible-secret"
            )
        visible.assert_called_once_with("请输入 COINGLASS_API_KEY: ")
    ...
    def test_real_profile_fails_closed_without_telegram(self) -> None:
        ...
        self.assertFalse((self.root / ".env.oi").exists())
    ...
    def test_environment_file_permission_is_600(self) -> None:
        self.manager.set("COINALYZE_API_KEY", "fake-secret")
        mode = (self.root / ".env.oi").stat().st_mode & 0o777
