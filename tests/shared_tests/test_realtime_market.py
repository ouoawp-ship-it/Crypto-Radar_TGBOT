...
class RealtimeMarketCliDispatchTests(unittest.TestCase):
    @patch(
        "shared.realtime_market.run_realtime_market_service",
        return_value=37,
    )
    def test_market_stream_cli_uses_shared_service(
        self,
        runner: Mock,
    ) -> None:
        from runtime.cli import main

        result = main(["market-stream", "--stream-duration-minutes", "0"])
...
class RealtimeFeatureAggregatorTests(unittest.TestCase):
    def test_builds_realtime_cvd_and_liquidation_boards(self) -> None:
        ...
        self.assertEqual(boards[0]["positive"]["items"][0]["value"], 500)
