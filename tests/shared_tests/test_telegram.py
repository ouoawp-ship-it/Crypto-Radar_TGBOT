...
class TelegramGatewayTests(unittest.TestCase):
    ...
    def test_remaining_alert_topic_intros_hold_static_guidance(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            expected = {
                "TG_FLOW_RADAR": [
                    "P2.0 使用单一1小时闭合窗口和 7 类互斥规则",
                    "取消 60～100 分评分",
                    "严格排除稳定币、TradFi 映射、身份不明资产和没有现货交易对的合约",
                    "超大/大/中/小市值 × 高/中/低流动性",
                    "快照完整时覆盖全部合格候选，逐币深度 REST 请求为0",
                    "永久保留至少四分之一名额用于公平轮换",
                    "异动过多时不会挤占轮换保底名额",
                    "dry-run 只预览，不推进生产轮换状态",
                    "分类图例",
