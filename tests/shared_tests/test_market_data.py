...
from shared.binance_data import BinanceDataSource, DataQuality, HttpClient, UpstreamSourceMetrics
...
class MarketCapSourceTests(unittest.TestCase):
    ...
    def test_http_cache_prunes_expired_entries_before_reuse(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            with patch("shared.binance_data.time.time", side_effect=lambda: now[0]):
