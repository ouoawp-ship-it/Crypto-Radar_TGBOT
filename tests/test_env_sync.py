...
class EnvSyncTests(unittest.TestCase):
    def test_sync_updates_managed_defaults_without_touching_secrets(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            example.write_text(
                "\n".join([
                    ...
                    "WEB_ADMIN_TOKEN=",
                    "OUTCOME_TRACKING_ENABLE=true",
                    "OUTCOME_DB_PATH=data/outcomes.db",
                    "OUTCOME_WINDOWS=1h,4h,24h,72h",
                    "OUTCOME_SCAN_LIMIT=100",
                    "OUTCOME_BACKFILL_DAYS=7",
                    "OUTCOME_PRICE_SOURCE=binance",
                    "OUTCOME_HTTP_TIMEOUT_SEC=10",
                    "OUTCOME_REQUEST_SLEEP_SEC=0.1",
                    "LIFECYCLE_TRACKER_ENABLE=true",
                    "LIFECYCLE_DB_PATH=data/lifecycle.db",
                    "LIFECYCLE_SCAN_INTERVAL_SEC=900",
                    "LIFECYCLE_LOOKBACK_HOURS=24",
                    ...
                    "LIFECYCLE_COOLING_PULLBACK_PCT=5",
        ...
        self.assertIn("OUTCOME_TRACKING_ENABLE=true", text)
        self.assertIn("OUTCOME_DB_PATH=data/outcomes.db", text)
        self.assertIn("OUTCOME_WINDOWS=1h,4h,24h,72h", text)
        self.assertIn("OUTCOME_SCAN_LIMIT=100", text)
        self.assertIn("OUTCOME_BACKFILL_DAYS=7", text)
        self.assertIn("OUTCOME_PRICE_SOURCE=binance", text)
        self.assertIn("OUTCOME_HTTP_TIMEOUT_SEC=10", text)
        self.assertIn("OUTCOME_REQUEST_SLEEP_SEC=0.1", text)
        self.assertIn("LIFECYCLE_TRACKER_ENABLE=true", text)
        self.assertIn("LIFECYCLE_DB_PATH=data/lifecycle.db", text)
        self.assertIn("LIFECYCLE_SCAN_INTERVAL_SEC=900", text)
        self.assertIn("LIFECYCLE_LOOKBACK_HOURS=24", text)
        ...
        self.assertIn("LIFECYCLE_COOLING_PULLBACK_PCT=5", text)
