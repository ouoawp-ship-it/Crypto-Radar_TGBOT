...
from paopao_radar.accumulation_diagnostics import (
    HISTORY_LIMIT,
    REASON_TEXT,
...
from paopao_radar.accumulation_quality import DAY_MS, analyze_accumulation_quality
...
class AccumulationRadarIntegrationTests(unittest.TestCase):
    ...
    def test_disabled_feature_creates_no_diagnostic_file_or_log(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            engine = RadarEngine(
                Settings(data_dir=root, accumulation_quality_v2_enable=False),
                JsonStore(root),
            )
    ...
    def test_write_failure_does_not_block_scan_result(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            engine = RadarEngine(
                Settings(data_dir=root, accumulation_quality_v2_enable=True),
                JsonStore(root),
            )
            window = closed_window(interval_sec=21600, delay_sec=300)
            stderr = io.StringIO()
