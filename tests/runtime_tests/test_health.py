...
class RuntimeHealthTests(unittest.TestCase):
    ...
    def test_retired_radar_diagnostics_do_not_affect_runtime_health(self) -> None:
        # Existing state files can retain old fields after a code upgrade.
        # A removed radar must neither be read nor restore a readiness gate.
        for historical_status in ("scan_failed", "commit_failed", "shadow_idle"):
            with self.subTest(historical_status=historical_status), TemporaryDirectory() as tmp:
                root = Path(tmp)
                now = 10_000
                settings = self.make_settings(root)
                store = JsonStore(root)
            ...
            runtime = next(item for item in checks if item["name"] == "runtime_status")
            self.assertEqual(runtime["status"], "ok")
            self.assertNotIn("hourly_proximity", json.dumps(runtime))
            self.assertNotIn("private body", json.dumps(runtime))

    def test_current_radar_failure_remains_blocking_with_retired_diagnostics(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            now = 10_000
            settings = self.make_settings(root)
        ...
        self.assertIn("pulse_failed", runtime["detail"])
