...
class MainBotUnitAndMenuTests(unittest.TestCase):
    ...
    def test_menu_exposes_only_explicit_topic_setup_with_dual_send_gate(self) -> None:
        text = MENU.read_text(encoding="utf-8")

        self.assertIn("手工创建/修复话题并置顶说明", text)
        self.assertIn('confirm_phrase "创建并置顶话题说明"', text)
        self.assertIn("telegram-topic-setup", text)
        self.assertIn("--send --confirm-real-send", text)
    ...
    def test_dry_run_prints_route_presence_without_route_ids(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            with (
                ...
                patch("shared.telegram.requests.post") as post_mock,
                patch.object(
                    gateway,
        ...
        self.assertIn("消息模板：TG_TEST_MESSAGE", text)
        self.assertIn("防重复标识：safe-dedup", text)
        self.assertIn("话题已配置：是", text)
        self.assertIn("回复目标已配置：是", text)
