...
# Retired templates cannot be revived by stale saved routes or direct callers.
RETIRED_TOPIC_TEMPLATE_IDS = frozenset({"TG_CONSOLIDATION_BREAKOUT"})
...
def topic_intro_message(template_id: str, settings: Settings) -> str:
    ...
    if template_id == "TG_FLOW_RADAR":
        ...
        "- P2.0 使用单一1小时闭合窗口和 7 类互斥规则；取消 60～100 分评分，避免把规则强弱伪装成精确概率。",
        "- 先用 Binance 元数据严格排除稳定币、TradFi 映射、身份不明资产和没有现货交易对的合约；不会只靠代码黑名单猜测。",
        "- 合格币按超大/大/中/小市值 × 高/中/低流动性使用不同有效门槛；市值缺失单列并采用保守门槛。",
        "- 优先聚合脉冲雷达写入统一市场快照的四段连续15分钟事实；快照完整时覆盖全部合格候选，逐币深度 REST 请求为0。",
        f"- 只有共享快照缺失的币才进入最多 {settings.flow_scan_limit} 个的 REST 回退：固定成交额头部最多 {settings.flow_fixed_top} 个，再加入价格/费率异动币，并永久保留至少四分之一名额用于公平轮换。",
        f"- 回退异动条件：24小时价格绝对变化≥{settings.flow_ticker_filter_pct:.2f}% 或资金费率绝对值≥{settings.flow_funding_filter_pct:.3f}%；异动过多时不会挤占轮换保底名额。",
        "- REST 回退中，同一闭合窗口的价格变化和合约主动成交净额复用一份 K 线；dry-run 只预览，不推进生产轮换状态。",
        "",
        "阅读方式：",
        ...
        "- 7 类按固定优先级互斥判定，同一币本轮只进入一个类别；展示按类别均衡取样，风险类不会被大量同类候选完全挤出。",
        "",
        "<b>消息保留规则</b>",
...
class TelegramGateway:
    ...
    def send(
        ...
        *,
        ...
        if template_id in RETIRED_TOPIC_TEMPLATE_IDS:
            return PushResult("blocked", "telegram_template_retired", False)
        if (
